...
<script type="text/javascript">
	$(function()
	{
		...
		$("#bootTable").bootstrapTable({
			url : Helper.basePath + "/finance/payment/ajaxDetailList",
			method : "post",
			...
			cookieIdTable : "print_finance_payment_detail",//必须制定唯一的表格cookieID
			...
			columns : [ {
				...
				formatter : function(value, row, index)
				{
					...
					return (row.master.isCheck == 'YES' ? '已审核' : '未审核') + (row.master.isCancel == 'YES' ? '(作废)' : '');
				},
				...
				field : 'master_currencyType',
				title : '币种',
				width : 80,
				formatter : function(value, row, index)
				{
					return row.master.currencyType;
				}
			}, {
				field : 'material',
				title : '材料/产品/工序',
			...
			onLoadSuccess : function()
			{
				...
				if ($(".glyphicon-th").next().html() == '')
				{
					...
					if (!Helper.basic.hasPermission('finance:payment_detail:export'))
					{
			...
			onDblClickRow : function(row)
			{
				...
				var url = Helper.basePath + '/finance/payment/view/' + row.master.id;
				var title = "付款单";
