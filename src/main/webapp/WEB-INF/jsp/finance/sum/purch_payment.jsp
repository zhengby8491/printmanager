...
<script type="text/javascript">
	$(function()
	{
		$("#isEmptyWare").prop("checked","checked");
		setQueryTime();
		...
		$("#bootTable").bootstrapTable({
			...
			onLoadSuccess : function(data)
			{
				...
				if ($(".glyphicon-th").next().html() == '')
				{
					...
					if (!Helper.basic.hasPermission('finance:sumpurchpayment:export'))
					{
			...
			//cookieIdTable : "print_finance_payment_purchShouldPayment_sum",//必须制定唯一的表格cookieID
			...
			columns : [ {
				...
				formatter : function(value, row, index)
				{
					return Number(Number(row.beginMoney).add(Number(row.shouldMoney)).add(Number(row.adjustMoney)).subtr(Number(row.discount)).subtr(Number(row.money))).tomoney();
				}
			}, {
				field : 'surAdvance',
				title : '预付余额',
				...
				formatter : function(value, row, index)
				{
					return Number(Number(row.beginMoney).add(Number(row.shouldMoney)).add(Number(row.adjustMoney)).subtr(Number(row.discount)).subtr(Number(row.money)).subtr(Number(row.surAdvance))).tomoney();
				}
			} ],
	...
	function queryParams(params)
	{
		...
		params['isEmptyWare'] = $("#isEmptyWare").is(':checked') == true ? "YES" : "NO";
		if ($("input[type='radio']:checked").val() != "")
		{
			params['supplierType'] = $("input[type='radio']:checked").val();
		}
		return params;
	}
		...
		<div class="page-border">
			...
			<div class="cl">
				<div class="iframe-top">
					<sys:nav struct="财务管理-账款汇总表"></sys:nav>
				</div>
			</div>
			<!--头部END-->
			<!--查询表单START-->
			...
			<div>
				<div class="radio-box">
					<label>
						<input type="radio" value="" name="supplierType" checked="checked" />
						全部
					</label>
					<label>
						<input type="radio" value="MATERIAL" name=supplierType />
						材料商
					</label>
					<label>
						<input type="radio" value="PROCESS" name="supplierType" />
						加工商
					</label>
					<label>
						<input type="radio" value="MATERIAL_AND_PROCESS" name="supplierType" />
						综合供应商
					</label>
				</div>
			</div>
