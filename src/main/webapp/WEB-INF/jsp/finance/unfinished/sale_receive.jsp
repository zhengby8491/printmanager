...
<script type="text/javascript">
	$(function()
	{
		...
		$("#bootTable").bootstrapTable({
			url : Helper.basePath + "/finance/unfinished/saleReceiveData",
			method : "post",
			...
			cookieIdTable : "print_finance_payment_shouldReceiveList",//必须制定唯一的表格cookieID
		...
		if (!Helper.basic.hasPermission('inance:unfinishedsalereceive:export'))
		{
		...
		$("#btn_transmit").click(function()
		{
			...
			if (Helper.isNotEmpty(ids))
			{
				...
				var url = Helper.basePath + '/finance/receive/create?1=1&billType=SALE_SK' + ids;
				var title = "收款单";
		...
		$("#btn_complete").on("click", function()
		{
			...
			{
				Helper.post(Helper.basePath + '/finance/unfinished/sale_complete', {
					"tableType" : "DETAIL",
					"ids[]" : ids
				}, function(data)
				{
		...
		$("#btn_complete_cancel").on("click", function()
		{
			...
			if (Helper.isNotEmpty(rows))
			{
				Helper.post(Helper.basePath + '/finance/unfinished/sale_complete_cancel', {
					"tableType" : "DETAIL",
					"ids[]" : ids
				}, function(data)
				{
		...
		<div class="page-border">
			...
			<div class="cl">
				<div class="iframe-top">
					<sys:nav struct="财务管理-应收账款明细"></sys:nav>
				</div>

				<div class="top_nav">
					<shiro:hasPermission name="finance:unfinished:sale_create">
						<button class="nav_btn table_nav_btn" id="btn_transmit">生成收款单</button>
					</shiro:hasPermission>

					<shiro:hasPermission name="finance:unfinished:sale_complete">
						<button class="nav_btn table_nav_btn" id="btn_complete">强制完工</button>
					</shiro:hasPermission>

					<shiro:hasPermission name="finance:unfinished:sale_cancel_complete">
				...
				<div class="cl order-info">
					...
					<dl class="cl hide_container" style="display: none;" id="more_div">
						<dd class="row-dd">
							...
							<span class="ui-combo-wrap form_text">
								<input id="saleOrderBillNo" name="saleOrderBillNo" type="text" class="input-txt input-txt_13" />
							</span>
						</dd>
						<dd class="row-dd">
							<label class="form-label label_ui">客户单号：</label>
							<span class="ui-combo-wrap form_text">
								<input id="customerBillNo" name="customerBillNo" type="text" class="input-txt input-txt_7" />
							</span>
						</dd>
						<dd class="row-dd">
							<label class="form-label label_ui">客户料号：</label>
							<span class="ui-combo-wrap form_text  wrap-width">
								<input id="customerMaterialCode" name="customerMaterialCode" type="text" class="input-txt input-txt_1" />
							</span>
						</dd>
