...
<script type="text/javascript">
	...
	function form_submit()
	{
		...
		Helper.request({
			url : Helper.basePath + "/finance/adjust/update",
			data : data,
	...
	function getCallInfo_business(rows, BUSINESSTYPE)
	{
		$.each(rows, function()
		{
			...
			{ // 判断是否重复，需外加判断是否同类型商家
				if ('${order.adjustType }' == BUSINESSTYPE)
				{
					return this.value;
				}
			}).get();
		...
		<div class="page-border">
			...
			<div class="cl">
				<div class="iframe-top">
					<sys:nav struct="财务管理-财务调整-编辑"></sys:nav>
				</div>
				<div class="top_nav">
					<shiro:hasPermission name="finance:adjust:edit">
						<button id="btn_save" class="nav_btn table_nav_btn">保存</button>
					</shiro:hasPermission>
					<shiro:hasManyPermissions name="finance:adjust:edit,finance:adjust:audit">
			...
			<form id="form_adjust" method="post">
							...
							<dl class="cl row-dl">
								<dd class="row-dd">
									<label class="form-label label_ui label_1">调整类型：</label>
										<input type="text" class="input-txt input-txt_1" readonly="readonly" id="adjustTypeText" value="${order.adjustTypeText }" />
										<input type="hidden" readonly="readonly" name="adjustType" id="adjustType" value="${order.adjustType }" />
									</span>
								</dd>
								<dd class="row-dd">
								...
								<dd class="row-dd">
									...
									<span class="ui-combo-wrap">
										<input id="adjustTime" name="adjustTime" type="text" class="input-txt input-txt_1 Wdate" onFocus="WdatePicker({lang:'zh-cn'})" 
											value="<fmt:formatDate pattern="yyyy-MM-dd" value='${order.adjustTime}' type='date' />" />
									</span>
								</dd>
								<dd class="row-dd">
									<label class="form-label label_ui label_1">调 整 人：</label>
									<span class="ui-combo-noborder">
										<phtml:list items="${fns:basicList2('EMPLOYEE')}" valueProperty="id" cssClass="input-txt input-txt_1 hy_select2"
											name="employeeId" textProperty="name" selected="${order.employeeId }"></phtml:list>
							...
							<dl class="cl row-dl">
								<dd class="row-dd">
									...
									<span class="form_textarea">
										<textarea name="memo" class="noborder" style="width: 996px" placeholder="说点什么...100个字符以内" 
											onKeyUp="Helper.doms.textarealength(this,100)">${order.memo }</textarea>
				...
				<div class="btn-billbar" id="toolbar">
					<button type="button" id="business_quick_select" class="nav_btn table_nav_btn">
						...
						<span id="btnName">
							<c:choose>
								<c:when test="${order.adjustType == 'PAY' }">选择供应商</c:when>
								<c:when test="${order.adjustType == 'RECEIVE' }">选择客户</c:when>
								<c:otherwise>选择账户</c:otherwise>
							</c:choose>
						</span>
					</button>
				</div>
						...
						<table class="border-table" style="width: 1000px">
							<thead>
								<tr>
									...
									<th width="40">操作</th>
									<th width="80">调整对象</th>
									<th width="140" class="code">编号</th>
									<th width="160" class="name">名称</th>
							...
							<tbody id="tbody">
								<c:forEach var="detail" items="${order.detailList }">
									...
									<td>
										<input type="hidden" readonly="readonly" name="detailList.id" value="${detail.id }">
										<input class="tab_input" readonly="readonly" type="text" value="${order.adjustTypeTarget}" />
									</td>
									<td>
										<input class="tab_input" readonly="readonly" type="text" name="detailList.businessCode" value="${detail.businessCode}" />
									</td>
									<td>
										<input class="tab_input" type="hidden" name="detailList.businessId" value="${detail.businessId}" />
										<input class="tab_input" readonly="readonly" type="text" name="detailList.businessName" value="${detail.businessName}" />
									</td>
									<td>
										<input class="tab_input bg_color constraint_decimal" type="text" name="detailList.adjustMoney" value="${detail.adjustMoney}" />
									</td>
									<td>
										<input class="tab_input bg_color memo" alt="调整事由" type="text" name="detailList.reason" value="${detail.reason}" />
									</td>
									<td>
										<input class="tab_input bg_color memo" type="text" name="detailList.memo" value="${detail.memo}" />
									</td>
									<tr>
								</c:forEach>
							</tbody>
