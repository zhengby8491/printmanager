...
$("#detailList")
		.bootstrapTable(
				{
					...
					cookieIdTable : "print_finance_writeoffReceive_master_view",//必须制定唯一的表格cookieID
	...
	$("#btn_back").on("click",function(){
		var url=Helper.basePath + '/finance/writeoffReceive/list';
		var title="收款核销单列表";
	...
	$("#btn_audit").on("click",function(){
		...
		Helper.post(Helper.basePath + '/finance/writeoffReceive/audit/' + order_id,function(data) {
			if(data.success){
				Helper.message.suc(data.message);
				location.href=Helper.basePath+"/finance/writeoffReceive/view/"+ order_id;
			}else
			{
	...
	$("#btn_audit_cancel").on("click",function(){
		var order_id=$("#order_id").val();
		Helper.post(Helper.basePath + '/finance/writeoffReceive/audit_cancel/' + order_id,function(data) {
			if(data.success){
				Helper.message.suc(data.message);
				location.href=Helper.basePath+"/finance/writeoffReceive/view/"+ order_id;
			}else
	...
	$("#btn_cancel").on("click",function(){
		var order_id=$("#order_id").val();
		Helper.post(Helper.basePath + '/finance/writeoffReceive/cancel/' + order_id,function(data) {
			if(data.success){
				Helper.message.suc(data.message);
				location.href=Helper.basePath+"/finance/writeoffReceive/view/"+ order_id;
			}else
	...
	$("#btn_cancel_back").on("click",function(){
		var order_id=$("#order_id").val();
		Helper.post(Helper.basePath + '/finance/writeoffReceive/cancel_back/' + order_id,function(data) {
			if(data.success){
				Helper.message.suc(data.message);
				location.href=Helper.basePath+"/finance/writeoffReceive/view/"+ order_id;
			}else
	...
	/* 打印*/
	$("#btn_print").loadTemplate('FINALCE_SKHX','/finance/writeoffReceive/printAjax/${id}');
		...
		<div class="page-border">
			...
			<div class="cl">
				<div class="iframe-top">
					<sys:nav struct="财务管理-收款核销单-查看"></sys:nav>
				</div>
				<div class="top_nav">
					...
					<c:if test="${order.isForceComplete eq 'NO'}">
						...
						<c:if test="${order.isCheck eq 'YES'}">
							<c:if test="${order.isCancel eq 'NO'}">
								<shiro:hasPermission name="finance:writeoffReceive:audit_cancel">
									<button class="nav_btn table_nav_btn" id="btn_audit_cancel">反审核</button>
								</shiro:hasPermission>
							</c:if>
						</c:if>
					</c:if>
					<shiro:hasPermission name="finance:writeoffReceive:print">
			...
			<div class="form-container">
				<div class="form-wrap">
					<div class="cl form_content">
						<dl class="cl row-dl">
							<dd class="row-dd">
								<label class="form-label label_ui label_1">核销单号：</label>
							...
							<dd class="row-dd">
								...
								<span class="ui-combo-wrap">
									<input type="text" class="input-txt input-txt_7" readonly="readonly" value="${order.customerName }" />
								</span>
							</dd>
							<dd class="row-dd">
						...
						<dl class="cl row-dl">
							...
							<dd class="row-dd">
								<label class="form-label label_ui label_1">核销金额：</label>
								<span class="ui-combo-wrap">
						...
						<dl class="cl row-dl">
							<dd class="row-dd">
								...
								<span class="form_textarea">
									<textarea class="noborder" readonly="readonly" style="width: 542px">${order.memo }</textarea>
								</span>
							</dd>
						</dl>
					</div>
