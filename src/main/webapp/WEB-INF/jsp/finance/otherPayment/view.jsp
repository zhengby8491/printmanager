...
<script type="text/javascript">
	$(function()
	{
		...
		$("#btn_audit").on("click",function(){
			...
			Helper.post(Helper.basePath + '/finance/otherPayment/audit/' + order_id,function(data) {
				if(data.success){
					Helper.message.suc(data.message);
					location.href=Helper.basePath+"/finance/otherPayment/view/"+ order_id;
				}else
				{
		...
		$("#btn_audit_cancel").on("click",function(){
			var order_id=$("#order_id").val();
			Helper.post(Helper.basePath + '/finance/otherPayment/audit_cancel/' + order_id,function(data) {
				if(data.success){
					Helper.message.suc(data.message);
					location.href=Helper.basePath+"/finance/otherPayment/view/"+ order_id;
				}else
		...
		$("#btn_edit").on("click",function(){
			var order_id=$("#order_id").val();
		 	var url = Helper.basePath + '/finance/otherPayment/edit/' + order_id;
			var title = "其它付款单";
			admin_tab($("<a _href='"+url+"' data-title='"+title+"' />"));
		});
		...
		$("#btn_del").on("click",function(){
			...
			{
				Helper.post(Helper.basePath + '/finance/otherPayment/del/' + order_id, function(data)
				{
					if (data.success)
					{
		...
		//打印模板加载
		$("#btn_print").loadTemplate('FINALCE_OFK', '/finance/otherPayment/printAjax/${order.id}');
		...
		<div class="page-border">
			...
			<div class="cl">
				<div class="iframe-top">
					<sys:nav struct="财务管理-其它付款单-查看"></sys:nav>
				</div>
				<div class="top_nav">
				...
				<div class="form-wrap">
					<div class="cl form_content">
						...
						<dl class="cl row-dl">
							...
							<dd class="row-dd">
								<label class="form-label label_ui label_1">付款单号：</label>
								<span class="ui-combo-wrap">
