...
<script type="text/javascript">
	$(function()
	{
		...
		$("#bootTable").bootstrapTable({
			url : Helper.basePath + "/finance/otherPayment/ajaxList",
			method : "post",
			...
			cookieIdTable : "print_finance_other_payment_master",//必须制定唯一的表格cookieID
			...
			columns : [ {
				...
				formatter : function(value, row, index)
				{
					...
					var operator = "";
					if (Helper.basic.hasPermission('finance:otherPayment:list'))
					{
						operator += '<a title="查看" href="javascript:;" onclick="payment_view(' + row.id 
								 + ')" style="padding: 0 8px; color: green"><i class="fa fa-search"></i></a>';
					}
					if(Helper.basic.hasPermission('finance:otherPayment:edit') && row.isCheck == "NO"){
						operator += '<a title="编辑" href="javascript:;" onclick="payment_edit('+ row.id
							     + ')" style="padding: 0 8px; color: green"><i class="fa fa-pencil"></i></a>';
					}
					if(Helper.basic.hasPermission('finance:otherPayment:delete') && row.isCheck == "NO")
					{
						operator += '<a title="删除" href="javascript:;" onclick="payment_delete(' + row.id 
								 + ')" style="padding: 0 8px; color: green" ><i class="delete fa fa-trash-o"></i></a>';
					}
					return operator;
				}
			} ],
			onLoadSuccess : function()
			{
				if ($(".glyphicon-th").next().html() == '')
				{
					...
					if (!Helper.basic.hasPermission('finance:otherPayment:export'))
					{
	...
	/* 新增其它付款单 */ 
	function create(){
		var url=Helper.basePath + '/finance/otherPayment/create';
		var title="其它付款单";
		admin_tab($("<a _href='"+url+"' data-title='"+title+"' />"));
	}
	/* 查看 */
	function payment_view(id) {
		var url=Helper.basePath + '/finance/otherPayment/view/' + id;
		var title="其它付款单";
		admin_tab($("<a _href='"+url+"' data-title='"+title+"' />"));
	}
	/* 编辑 */
	function payment_edit(id){
		var url=Helper.basePath + '/finance/otherPayment/edit/' + id;
		var title="其它付款单";
		admin_tab($("<a _href='"+url+"' data-title='"+title+"' />"));
	}
	/* 删除*/
	function payment_delete(id){
    	Helper.message.confirm('确认要删除吗？', function(index) {
			Helper.post(Helper.basePath + '/finance/otherPayment/del/' + id,function(data) {
				if(data.success){
					Helper.message.suc("已删除！");
					$("#bootTable").bootstrapTable("refreshOptions",{pageNumber:1});
					/* var url=Helper.basePath + '/finance/otherPayment/list/';
					var title="其它付款单列表";
					admin_tab($("<a _href='"+url+"' data-title='"+title+"' />")) */
				}else
				{
		...
		<div class="page-border">
			...
			<div class="cl">
				<div class="cl order-info">
					<dl class="cl row-dl">
						...
						<dd class="row-dd">
							<label class="form-label label_ui">收款单位：</label>
			...
			<div class="btn-bar" style="margin-bottom: 0">
				<shiro:hasPermission name="finance:otherPayment:create">
					<span>
						<a href="javascript:;" onclick="create()" class="nav_btn table_nav_btn">
