...
<body>
	...
	<script id="itemlisttmpl" type="text/x-dot-template">
					{{ for(var k in it) { }}
                    <li class="mar-btm6" data-index="{{= it[k].index}} data-id="{{= it[k].id}}">                      
				      <div class="item">  
				          <div class="item-kv">
				            <div class="item-key">发外单号 :</div>
				            <div class="item-value w-60p">{{= it[k].billNo||'' }}</div>
				          </div>
				          <div class="item-kv">
							{{? it[k].productName}}
				            	<div class="item-key">成品名称 :</div>							
				            	<div class="item-value w-60p">{{= it[k].productName||'' }}({{= it[k].style||'' }})</div>
							{{?? it[k].procedureName}}
								<div class="item-key">工序名称 :</div>							
				            	<div class="item-value w-60p">{{= it[k].procedureName||'' }}({{= it[k].style||'' }})</div>
							{{?}}
				          </div>
				          <div class="row no no-gutter item_detail">	
							<div class="color-o col-20">￥{{=it[k].price}}</div>
							<div class="col-10">x</div>
							<div class="col-33">{{=it[k].qty}}{{=it[k].unitName||'张'}}</div>
        ...
        {                            
            ...
            function addItems(){
                HYWX.loadMore.addData({
                    url:'${ctx}/wx/sum/outsourceSupplierSumDetail',
