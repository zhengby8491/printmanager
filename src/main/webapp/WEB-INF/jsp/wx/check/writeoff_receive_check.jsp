	...
	<div class="page page-current">
		<div class="searchbar">
			...
			<div class="search-input">
				...
				<input type="search" id="search" placeholder="搜索核销单号/客户名称">
			</div>
		</div>
					...
					<label class="label-checkbox">		
						<div class="check_item_head cl">							
							<div class="item-kv pull-left">
						            <div class="item-key w-56">核销单号  :</div>
						        		...
						        		<div style="padding: 0 0.2rem 0 1.3rem;">			        			
									        <div class="item-kv">
									         ...
									         <div class="item-bottom" style="padding-bottom: 0.1rem;">
												<div class="pull_left">
									            	<div class="item-key">核销金额  :</div>
									            	<div class="item-value color-o">￥<span class="item_money">{{=it[k].money}}</span></div>
												</div>
												<div class="pull_left">
									            	<div class="item-key">(折扣金额  :</div>
									            	<div class="item-value color-o">￥<span class="item_money">{{=it[k].discount}}</span>)</div>
												</div>
									         </div>
									     </div>
									     <!--  选中按钮 -->
	...
	<script type="text/javascript">	    
        ...
        {                            
            ...
            function addItems(){
                HYWX.loadMore.addData({
                    url:'${ctx}/wx/check/writeoffReceiveCheckList',
                    data:{
        ...
        $(document).on("click",".audit_btn",function(){
        	...
        	$.confirm('确认审核?', '',function (){
	        	 HYWX.request({
	                url:'${ctx}/wx/check/writeoffReceiveCheck',
	                data:{"id":billId},
	                dataType:"json",
        	...
        	if($(this).hasClass("active")){
        		$.confirm('确认审核?', '',
        		        ...
        		        function () {
        		          ...
        		          HYWX.requestByObj({
                              url:'${ctx}/wx/check/writeoffReceiveCheckAll',
                              data:{"ids":ids},
                              success:function(data){
        ...
        function addDetail($a){
        		...
        		if($a.find(".check_item_td").children().eq(0).hasClass("check_detail_preloader")){
        			HYWX.requestByObj({
                        url:'${ctx}/wx/check/writeoffReceiveCheckDetail',
                        data:{"id":billId},
                        success:function(data){
