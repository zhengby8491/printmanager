	...
	<div class="page page-current">
		<div class="searchbar">
			...
			<div class="search-input">
				...
				<input type="search" id="search" placeholder="搜索收款单号/客户名称">
			</div>
		</div>
					...
					<label class="label-checkbox">		
						<div class="check_item_head cl">							
							<div class="item-kv pull-left">
						            <div class="item-key w-56">收款单号  :</div>
...
<!-- 						            <span><i class="icon iconfont icon-nz"></i></span> -->
						...
						<table>
							<tbody>
									...
									<td colspan="3" class="check_item_content bg-w">							            	            
						        		<div style="padding: 0 0.2rem 0 1.3rem;">			        			
									        <div class="item-kv">
									            ...
									            <div class="item-value">{{=it[k].customerName||''}}</div>
									        </div>
									         <div class="item-bottom" style="padding-bottom: 0.1rem;">
												<div>
									            	<div class="">结算方式  :</div>
									            	<div class="">{{=it[k].settlementClassName||''}}</div>
												</div>
												<div>
									            	<div style="margin-left:.5rem">收款天数  :</div>
									            	<div class="">{{=it[k].billDay}}</div>
												</div>
									         </div>
									     </div>
									     <!--  选中按钮 -->
			        			...
			        			<tr class="bg-w">
				        			<td class="check_money_td">收款金额:<span class="color-o">￥<span class="item_money">{{=it[k].money}}</span></span>
				        			<span class="">(折扣:<span class="color-o">￥<span>{{=it[k].discount}}</span></span></span>，
									<span class="">预收:<span class="color-o">￥<span>{{=it[k].advance}}</span></span>)</span></td>
				        		</tr>
										...
										<div class="check_detail_content">
					        				...
					        				<div class="check_item_detail">
					        					...
					        					<div class="cl">
													...
													<div class="item-kv pull-left">
														<span class="item-key">(源单金额  :</span>
											        	<span class="item-value color-o">￥{{=it[k].sourceMoney}}</span>	)
													</div>										        
					        					</div>
					        					<div class="cl">				        						
											        <div class="item-kv pull-left">										            										          																
											            <span class="item-value">待收  :</span>
														<span class="item-value color-o mar-r6">￥{{=it[k].sourceBalanceMoney}}</span>														
											        </div>	
													<div class="pull-right w-120">
														<span class="item-value">本次收  :</span>
														<span class="item-value color-o">￥{{=it[k].money}}</span>	
													</div>												
					        					</div>
					        				</div>
											{{ } }}
	...
	<script type="text/javascript">	    
        ...
        {                            
            ...
            function addItems(){
                HYWX.loadMore.addData({
                    url:'${ctx}/wx/check/receiveCheckList',
                    data:{
        	...
        	if($(this).hasClass("active")){
        		$.confirm('确认审核?', '',
        		        ...
        		        function () {
        		          ...
        		          HYWX.requestByObj({
                              url:'${ctx}/wx/check/receiveCheckAll',
                              data:{"ids":ids},
                              success:function(data){
        ...
        function addDetail($a){
        		...
        		if($a.find(".check_item_td").children().eq(0).hasClass("check_detail_preloader")){
        			HYWX.requestByObj({
                        url:'${ctx}/wx/check/receiveCheckDetail',
                        data:{"id":billId},
                        success:function(data){
