	...
	<div class="page page-current">
		<div class="searchbar">
			...
			<div class="search-input">
				...
				<input type="search" id="search" placeholder="搜索付款单号/供应商名称">
			</div>
		</div>
					...
					<label class="label-checkbox">		
						<div class="check_item_head cl">							
							<div class="item-kv pull-left">
						            <div class="item-key w-56">付款单号  :</div>
...
<!-- 						            <span><i class="icon iconfont icon-nz"></i></span> -->
						...
						<table>
							<tbody>
						        		...
						        		<div style="padding: 0 0.2rem 0 1.3rem;">			        			
									        <div class="item-kv">
									         ...
									         <div class="item-bottom" style="padding-bottom: 0.1rem;">
												...
												<div>
									            	<div style="margin-left:.5rem">剩余付款天数  :</div>
			        			...
			        			<tr class="bg-w">
				        			<td class="check_money_td">付款金额  :<span class="color-o">￥<span class="item_money">{{=it[k].money}}</span></span>
				        			<span class="">(折扣  :<span class="color-o">￥<span>{{=it[k].discount}}</span></span></span>，
									<span class="">预付  :<span class="color-o">￥<span>{{=it[k].advance}}</span></span>)</span></td>
				        		</tr>
	...
	<script id="detaillisttmpl" type="text/x-dot-template">
										<div class="check_detail_content">
					        				...
					        				<div class="check_item_detail">
					        					...
					        					<div class="cl">
					        						 <div class="item-kv pull-left">
											            <div class="item-value">{{=it[k].name||''}}</div>  
											        </div>
													<div class="item-kv pull-left">
	...
	<script type="text/javascript">	    
        ...
        {                            
            ...
            function addItems(){
                HYWX.loadMore.addData({
                    url:'${ctx}/wx/check/paymentCheckList',
                    data:{
        	...
        	if($(this).hasClass("active")){
        		$.confirm('确认审核?', '',
        		        ...
        		        function () {
        		          ...
        		          HYWX.requestByObj({
                              url:'${ctx}/wx/check/paymentCheckAll',
                              data:{"ids":ids},
                              success:function(data){
        ...
        function addDetail($a){
        		...
        		if($a.find(".check_item_td").children().eq(0).hasClass("check_detail_preloader")){
        			HYWX.requestByObj({
                        url:'${ctx}/wx/check/paymentCheckDetail',
                        data:{"id":billId},
                        success:function(data){
                            if(data=="turn"){
