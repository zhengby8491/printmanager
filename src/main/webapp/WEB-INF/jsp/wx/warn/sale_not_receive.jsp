                    ...
                    <li class="mar-btm6" data-index="{{= it[k].index}}">
                      <div class="item-content">
				          ...
				          <div class="item-ordernum">
				            <div class="item-media color-g w-69">销售单号 :</div>
				            <div class="item-inner">{{= it[k].saleBillNo }}</div>
				          </div>
				      </div>
				      <div class="item">  
				          <div class="item-kv">
				            <div class="item-key w-69 ">成品名称 :</div>
				            <div class="item-value color-o w-55p">{{= it[k].productName }}</div>
				          </div>
				          <div class="item-kv">
				            <div class="item-key w-69">客户名称 :</div>
				            <div class="item-value color-o w-55p">{{= it[k].customerName }}</div>
				          </div>
						  <div class="item-kv">
				            <div class="item-key w-69">销售数量 :</div>
				            <div class="item-value color-o w-55p">{{= it[k].saleQty }}{{= it[k].unitName||'' }}</div>
				          </div>
				          <div class="item-bottom">
							<div style="">
				            	<div class="w-69">销售金额 :</div>
				            	<div class="w_num_unit color-o">￥{{= it[k].saleMoney }}</div>
							</div>
							<div>
				            	<div style="margin-left:.5rem">已收金额:</div>
				            	<div class="color-o">￥{{= it[k].receiveMoney }}</div>
							</div>
				          </div>
							<div class="circle" data-pct="{{= it[k].reconcilPercent }}">
    							<div class="pie_left"><div class="left"></div></div>
    							<div class="pie_right"><div class="right"></div></div>
    							<div class="mask">
									<div>收款天数</div>
									<span class="receiveDay">{{=it[k].receiveDay}}</span><span class="color-b">天</span>
								</div>
  							</div>
				       </div>
                    </li>
        ...
        {                            
            ...
            function addItems(){
                HYWX.loadMore.addData({
                    url:'${ctx}/wx/warn/saleNotReceive',
                    data:{
                    ...
                    success:function(data){
                        ...
                        for(var i=startIndex;i<endIndex;i++){
                          	$new.push($("#itemlist li").eq(i-1).find(".circle"));
                        }
