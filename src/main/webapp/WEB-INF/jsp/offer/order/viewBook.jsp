		...
		<div class="page-border">
			...
			<div id="offer_detail_content">
				<table id="show_table" border="0" class="border-table offer_detail_table">
					<thead>
						<td colspan="14" style="background: #e2e9ef;">
					...
					<tbody>
						<tr>
							<td rowspan="2" width="70px">成品信息</td>
							<td>客户名称</td>
							<td>客户地址</td>
							<td>联系电话</td>
							<td>成品名称</td>
							<td colspan="2">成品规格</td>
							<td colspan="2">报价数量</td>
							<td colspan="2">报价单号</td>
							<td>交货日期</td>
							<td>报价人</td>
							<td>报价日期</td>
						</tr>
						<tr class="whiteBg">
							...
							<td>${offerBean.productName}</td>
							<td colspan="2">${offerBean.spec}</td>
							<td colspan="2">${offerBean.amount}</td>
						...
						<c:forEach items="${offerBean.offerPartList }" var="partList">
							<tr>
								<td rowspan="5">${partList.partName}</td>
								<td>P数</td>
								<td>印刷机</td>
								<td>上机规格</td>
								<td>印刷方式</td>
								<td style="width: 5%">正反普色</td>
								<td style="width: 5%">正反专色</td>
								<td style="width: 5%">印版付数</td>
								<td style="width: 5%">印版张数</td>
								<td style="width: 5%">单面拼数</td>
								<td style="width: 5%">手/贴数</td>
								<td>印张正数</td>
								<td>放损数</td>
								<td>总印张</td>
							</tr>
							<tr class="whiteBg">
								<td>${partList.pages}</td>
								...
								<td>${partList.offerPrintStyleType.value * partList.thread}</td>
								<td>${partList.sheetZQ}</td>
								<td>${partList.sheetNum}</td>
								<td>${partList.thread}</td>
							...
							<tr>
								...
								<td>材料规格</td>
								<td>克重</td>
								<td colspan="2">上机数量</td>
								<td colspan="2">材料开数</td>
							...
							<tr class="whiteBg">
								...
								<td>${partList.paperWeight}</td>
								<td colspan="2">${partList.paperTotal}</td>
								<td colspan="2">${partList.materialOpening}</td>
								<td colspan="2">${partList.materialAmount}</td>
							...
							<tr class="whiteBg">
								<td>后道工序</td>
								<td colspan="12" style="text-align: left; word-wrap: break-word; white-space: normal;">${partList.partProcedureStr}</td>
							</tr>
						</c:forEach>
						<tr class="whiteBg">
							<td style="background: #F1F1F1">成品工序</td>
							<td>成品工序</td>
							<td colspan="12" style="text-align: left; word-wrap: break-word; white-space: normal;">${offerBean.productProcedureStr}</td>
						</tr>
					</tbody>
				</table>
				...
				<table border="0" class="border-table offer_detail_table" style="margin-top: 0px;">
					<tbody>
						<tr>
							<td rowspan="${fn:length(offerBean.offerOrderQuoteInnerList)+1 }" width="70px;">阶梯数据</td>
							<td>数量</td>
							<td>纸张费</td>
							<td>印刷费</td>
							<td>工序费</td>
