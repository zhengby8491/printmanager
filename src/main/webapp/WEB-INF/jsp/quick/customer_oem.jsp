...
<script type="text/javascript">
	...
	$(function()
	{
		...
		$("#btn_ok").click(function()
		{
			if ($("#multiple").val().toBoolean())
			{//多选,则双击事件
				parent.getCallInfo_customerOemArray($("#bootTable").bootstrapTable('getAllSelections'));
			}
			Helper.popup.close();
		});
		...
		$("#bootTable").bootstrapTable({
			url : Helper.basePath + "/quick/customerOemList",
			method : "post",
	...
	function selectRow(row)
	{
		...
		if ($("#multiple").val().toBoolean())
		{//多选,则双击事件无效
			parent.getCallInfo_customerOemArray([ row ]);
		} else
		{
			// 如果供应商分类为null，则提示需要创建供应商
			if (row.customerClassName == null || row.customerClassName == "-")
			{
				...
				} else
				{
					...
					{
						quickCustomerAdd(row.name, row.defaultAddress.address, row.defaultAddress.userName, row.defaultAddress.mobile, row.originCompanyId);
						layer.close(index);
					});
				}
			...
			{
				parent.getCallInfo_customerOem(row);
				Helper.popup.close();
			}
		}
	}
...
<body>
	...
	<input type="hidden" id="zeroOriginType" value="CUSTOMER" />
	<div class="layer_container">
		<div class="cl layer_content">
			<!--分类选择-->
			<div class="tree">
				<ul>
					<a href="javascript:void(0);" _id="CUSTOMER" name="zeroOriginType" style="color: red;">客户</a>
