...
<meta charset="UTF-8">
<title>选择供应商代工平台</title>
<script type="text/javascript">
	//========= 初始化区域 =========
	function addressInit(_cmbProvince, _cmbCity, _cmbArea, defaultProvince, defaultCity, defaultArea, _triggerChange)
	{
	...
	$(function()
	{
		...
		// 区域选择
		// 		$("#cmbProvince, #cmbCity, #cmbArea").on("change", function()
		// 		{
		...
		// 供应商、代工平台选择
		$(".tree").on("click", "a[name='zeroOriginType']", function()
		{
			$(".tree a[name='zeroOriginType']").css("color", "");
			$(this).css("color", "red");
		...
		$("#btn_ok").click(function()
		{
			if ($("#multiple").val().toBoolean())
			{//多选,则双击事件
				parent.getCallInfo_supplierOemArray($("#bootTable").bootstrapTable('getAllSelections'));
			}
			Helper.popup.close();
		});
		...
		{
			quickSupplierAdd("PROCESS");
		});
		//	console.log($("#multiple").val().toBoolean());
		//订单详情table
		$("#bootTable").bootstrapTable({
			url : Helper.basePath + "/quick/supplierOemList",
			method : "post",
	...
	function selectRow(row)
	{
		if ($("#multiple").val().toBoolean())
		{
			parent.getCallInfo_supplierOemArray([ row ]);
		} else
		{
			// 如果供应商分类为null，则提示需要创建供应商
			if (row.supplierClassName == null || row.supplierClassName == "-")
			{
				// 检查基础资料是否存在代工平台供应商（用于代工平台）
				if (row.originCompanyExit === 'YES')
				{
					if (row.type == "MATERIAL")
					{
						Helper.message.view("供应商名称已存在，但不属于加工商，请修改之后刷新页面再操作");
						return false;
					}

					Helper.message.confirm("供应商名称已存在，是否与代工平台供应商绑定？", function(index)
					{
						var _data = {
							id : row.id,
							name : row.name,
						...
						Helper.request({
							url : Helper.basePath + "/basic/supplier/updateOem",
							data : _data,
							async : false,
				...
				{
					Helper.message.confirm("是否需要创建代工平台供应商", function(index)
					{
						quickSupplierAdd("PROCESS", row.name, row.defaultAddress.address, row.defaultAddress.userName, row.defaultAddress.mobile, row.originCompanyId);
						layer.close(index);
					});
				}
			} else
			{
				parent.getCallInfo_supplierOem(row);
				Helper.popup.close();
			}
		}
	}
	...
	{
		// 源数据来源类型（暂用于代工平台）
		params['zeroOriginType'] = $("#zeroOriginType").val();
		// 供应商名称
		params['supplierName'] = $("#supplierName").val();

		// 公司过滤区域
		var _originType = $("#zeroOriginType").val();
		if (_originType == "COMPANY")
		{
			params['province'] = $("#cmbProvince").val() == "请选择" ? "" : $("#cmbProvince").val();
			params['city'] = $("#cmbCity").val() == "请选择" ? "" : $("#cmbCity").val();
			params['county'] = $("#cmbArea").val() == "请选择" ? "" : $("#cmbArea").val();
		}
	 ...
	 * @since 1.0, 2018年3月16日 下午3:13:32, think
	 */
	function quickSupplierAdd(defaultSupplierType, supplierName, supplierAddress, supplierLinkName, supplierMobile, originCompanyId)
	{
		var add_url = Helper.basePath + "/quick/supplier_add?";
		if (Helper.isNotEmpty(defaultSupplierType))
		{
			add_url = add_url + "defaultSupplierType=PROCESS";
		}
		if (Helper.isNotEmpty(supplierName))
		{
			add_url = add_url + "&supplierName=" + supplierName;
		}
		if (Helper.isNotEmpty(supplierAddress))
		{
			add_url = add_url + "&supplierAddress=" + supplierAddress;
		}
		if (Helper.isNotEmpty(supplierLinkName))
		{
			add_url = add_url + "&supplierLinkName=" + supplierLinkName;
		}
		if (Helper.isNotEmpty(supplierMobile))
		{
			add_url = add_url + "&supplierMobile=" + supplierMobile;
		}
...
<body>
	...
	<input type="hidden" id="zeroOriginType" value="SUPPLIER" />
	<div class="layer_container">
		<div class="cl layer_content">
			<!--分类选择-->
			<div class="tree">
				<ul>
					<a href="javascript:void(0);" _id="SUPPLIER" name="zeroOriginType" style="color: red;">供应商</a>
					<a href="javascript:void(0);" _id="COMPANY" name="zeroOriginType">代工平台</a>
				</ul>
			</div>
			<!--表格容器左START-->
			<div class="layer_table_container wrap_width_1">
				<div class="cl layer_top">
					<div class="row-dd top_bar">
						...
						<span class="ui-combo-wrap" id="areaChoose" style="padding-left: 5px; display: none;">
							<label class="">区域：</label>
							<select class="m_input hy_select2" id="cmbProvince" name="province" style="width: 100px;"></select>
							<select class="m_input hy_select2" id="cmbCity" name="city" style="width: 100px;"></select>
							<select class="m_input hy_select2" id="cmbArea" name="county" style="width: 100px;"></select>
						</span>
