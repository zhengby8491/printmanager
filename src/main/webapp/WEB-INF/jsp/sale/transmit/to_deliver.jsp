...
<script type="text/javascript" src="${ctxStatic}/site/sale/transmit/to_deliver.js?v=${v}"></script>
<title>订单转送货</title>
</head>
<body>
			...
			<div class="cl">
				<div class="iframe-top">
					<c:if test="${fns:hasCompanyPermission('sale:order:list')}">
						<sys:nav struct="销售管理-转单功能-销售单转送货单"></sys:nav>
					</c:if>

					<c:if test="${!fns:hasCompanyPermission('sale:order:list')}">
						<sys:nav struct="销售管理-转单功能-生产单转送货单"></sys:nav>
					</c:if>

				</div>
				<div class="top_nav">
					<shiro:hasPermission name="sale:deliver:create">
						<button class="nav_btn table_nav_btn" id="btn_transmit">生成销售送货单</button>
					</shiro:hasPermission>
