...
<title>编辑送货单</title>
<script type="text/javascript" src="${ctxStatic}/site/sale/deliver/edit.js?v=${v}"></script>
</head>
<body>
		...
		<div class="page-border">
			...
			<div class="cl">
				<div class="iframe-top">
					<sys:nav struct="销售管理-销售送货-编辑"></sys:nav>
				</div>
				...
				<div class="top_nav">
					<shiro:hasPermission name="sale:deliver:edit">
						<button class="nav_btn table_nav_btn" id="btn_save">保存</button>
					</shiro:hasPermission>
					<shiro:hasManyPermissions name="sale:deliver:edit,sale:deliver:audit">
			...
			<form id="form_payment">
						...
						<div class="cl form_content">
							<dl class="cl row-dl">
								...
								<dd class="row-dd">
									...
									<span class="ui-combo-wrap">
										<input type="text" name="deliveryTime" class="input-txt input-txt_3 Wdate" onFocus="WdatePicker({lang:'zh-cn'})" value="<fmt:formatDate value="${order.deliveryTime}" type="date" pattern="yyyy-MM-dd" />" />
									</span>
								</dd>
								<dd class="row-dd">
							...
							<dl class="cl row-dl">
								...
								<dd class="row-dd">
									...
									<span class="ui-combo-wrap">
										<input type="text" readonly="true" class="input-txt input-txt_3" value="${order.billNo }" />
									</span>
								</dd>
								<dd class="row-dd">
							...
							<tbody>
								<c:forEach var="detail" items="${order.detailList}" varStatus="status">
									<tr name="tr">
										...
										<%-- </shiro:hasPermission> --%>
										<td>
											<phtml:list items="${fns:basicList2('TAXRATE')}" valueProperty="id" cssClass="tab_input bg_color" textProperty="name" name="detailList.taxRateId" selected="${detail.taxRateId }" />
										</td>
										<td style="display: none">
										...
										<td>
											<phtml:list items="${fns:basicListParam2('WAREHOUSE', 'warehouseType','PRODUCT')}" valueProperty="id" name="detailList.warehouseId" onchange="shotCutWindow('WAREHOUSE',true,$(this),'PRODUCT')" textProperty="name" cssClass="tab_input bg_color" selected="${detail.warehouseId }"></phtml:list>
										</td>
										<td style="display: none">
				...
				<div class="cl form_content">
					...
					<dl class="cl row-dl-foot">
						<dd class="row-dd">
							...
							<span class="ui-combo-wrap">
								<input type="text" class="input-txt input-txt_3" readonly value="${order.createName}" />
							</span>
						</dd>
						<dd class="row-dd">
						...
						<dd class="row-dd">
							...
							<span class="ui-combo-wrap">
								<input name="checkTime" type="text" class="input-txt input-txt_3" readonly value="${order.checkTime }" />
							</span>
						</dd>
					</dl>
				</div>
				...
				<div id="batch_taxRate_box" class="batch_taxRate_box">
					<phtml:list items="${fns:basicList2('TAXRATE')}" cssClass="tab_input bg_color  input-txt_3 hy_select2 batch_taxRate_select" valueProperty="id" textProperty="name" name="rateId" selected="${order.rateId }" onchange="batchEditTaxRate()" />
				</div>
				<!-- 批量悬浮窗 end -->
			</form>
		</div>
