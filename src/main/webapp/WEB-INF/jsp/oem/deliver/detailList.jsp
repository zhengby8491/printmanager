...
<script type="text/javascript">
	...
	{
		/* 判断当前用户是否有权限查看金额 */
		var hasPermission = Helper.basic.hasPermission('oem:deliver:detail:money');
		/* 选择客户 */
		...
		{
			Helper.popup.show('选择工序', Helper.basePath + '/quick/product_select?multiple=false', '900', '490');
		});
		...
		$("#bootTable").bootstrapTable({
			url : Helper.basePath + "/oem/deliver/ajaxDetailList",
			method : "post",
			...
			cookieIdTable : "print_oem_deliver_detail",//必须制定唯一的表格cookieID
			...
			columns : [ {
				...
				formatter : function(value, row, index)
				{
					return (row.qty - row.returnQty).toFixed(2);
				}
			}, {
				field : 'reconcilQty',
				title : '已对账数量',
			...
			onDblClickRow : function(row)
			{
				...
				var url = Helper.basePath + '/oem/deliver/view/' + row.master.id;
				var title = "代工送货";
		...
		$("#bootTable").on('load-success.bs.table column-switch.bs.table page-change.bs.table search.bs.table', function()
		{
			...
			if ($(".glyphicon-th").next().html() == '')
			{
				...
				if (!Helper.basic.hasPermission('oem:deliver:detail:export'))
				{
				...
				<div class="cl order-info">
					...
					<dl class="cl row-dl" style="display: none;" id="more_div2">
						<dd class="row-dd">
							...
							<span class="ui-combo-wrap wrap-width">
								<input type="text" class="input-txt input-txt_23" id="productName" name="productName" />
