...
<script type="text/javascript">
	...
	$(function()
	{
		if (!hasPermission)
		{
			...
			$("#detailList").find("th[name='noTaxPrice']").hide();
			$("#detailList").find("th[name='noTaxMoney']").hide();
	...
	function appendTr(tableId, rows)
	{
		$.each(rows, function()
		{
			...
			copyTr.find("select[name='detailList.taxRateId']").val(_THIS.taxRateId);
			copyTr.find("input[name='detailList.percent']").val(_THIS.percent);
	...
	function form_submit()
	{
		...
		// 提交到服务器保存
		Helper.request({
			url : Helper.basePath + "/oem/order/update",
		...
		<div class="page-border">
			...
			<div class="cl">
				<div class="iframe-top">
					<sys:nav struct="代工管理-代工订单-编辑"></sys:nav>
				</div>
				<div class="top_nav">
					<shiro:hasPermission name="oem:order:edit">
						<button class="nav_btn table_nav_btn" id="btn_save">保存</button>
					</shiro:hasPermission>
					<shiro:hasManyPermissions name="oem:order:edit,oem:order:audit">
							...
							<dl class="cl row-dl">
								...
								<dd class="row-dd">
									...
									<span class="ui-combo-wrap" class="form_text">
										<input type="text" class="input-txt input-txt_3" name="billNo" id="billNo" readonly="readonly" class="input-txt input-txt_3" value="${order.billNo }" />
									</span>
								</dd>
							</dl>
							<dl class="cl row-dl">
							...
							<dl class="cl row-dl">
								<dd class="row-dd">
									...
									<span class="form_textarea">
										<textarea name="memo" id="memo" class="noborder" style="width: 953px" placeholder="说点什么...100个字符以内" onKeyUp="Helper.doms.textarealength(this,100)">${order.memo }</textarea>
						...
						<table class="border-table" style="width: 1450px" id="detailList">
							<thead>
								<tr>
									...
									<th width="60" name="productName">成品名称</th>
									<th width="60" name="originProcedureName">源单工序名称</th>
									<th width="60" name="procedureName">工序名称</th>
									<th width="40" name="specifications">加工规格</th>
									<th width="60">源单单号</th>
									<th width="40">源单数量</th>
									<th width="40">生产数量</th>
									<th width="40" name="qty">加工数量</th>
									<th width="30" name="price">单价</th>
									<th width="30" name="money">金额</th>
									<th width="30" name="tax">税额</th>
									<th width="50" name="taxRateId">
										税收
									...
									<th width="55" name="deliveryTime">
										交货日期
										<i id="batch_edit_deliveryTime" class="fa fa-edit" src="batch_deliveryTime_box"></i>
										<div class="batch_box_container"></div>
									</th>
									<shiro:hasPermission name="oem:order:money">
										<th width="40" name="noTaxPrice">不含税单价</th>
										<th width="40" name="noTaxMoney">不含税金额</th>
									</shiro:hasPermission>
									<th width="40">部件名称</th>
									<th width="60 " name="processRequire">加工要求</th>
									<th width="40 " name="memo">备注</th>
								</tr>
							</thead>
							<tbody>
					...
					<dl class="cl row-dl-foot">
						...
						<dd class="row-dd">
							...
							<span class="ui-combo-wrap" class="form_text">
								...
								<input id="currencyTypeText" type="text" class="input-txt input-txt_3" readonly="readonly" value="${order.currencyType.text }" />
							</span>
						</dd>
					</dl>
					<dl class="cl row-dl-foot">
	...
	<div id="cloneTR" style="display: none;">
		<table>
			<tr>
				...
				<!-- 数量 -->
				<td>
					<input class="tab_input bg_color constraint_decimal" type="text" name="detailList.qty" value="0" />
					<input type="hidden" name="detailList.saveQty" value="" />
				</td>
				<!-- 单价 -->
				<td>
					<input class="tab_input bg_color constraint_decimal" type="text" name="detailList.price" value="0" />
				</td>
				<!-- 金额 -->
				<td>
				...
				<!-- 交货日期 -->
				<td>
					<input class="tab_input bg_color" type="text" name="detailList.deliveryTime" onFocus="WdatePicker({lang:'zh-cn',minDate: '%y-%M-%d' })" value="<fmt:formatDate value="${now}" pattern="yyyy-MM-dd" />" />
				</td>
				<!-- 不含税单价 -->
				<td>
