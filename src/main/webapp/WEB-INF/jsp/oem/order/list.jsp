...
<script type="text/javascript">
	$(function()
	{
		...
		//判断当前用户是否有权限查看金额
		var hasPermisson = Helper.basic.hasPermission('oem:order:money');
		// 订单详情table
		$("#bootTable").bootstrapTable({
			url : Helper.basePath + "/oem/order/ajaxList",
			method : "post",
			...
			cookieIdTable : "print_oem_order_master",//必须制定唯一的表格cookieID
			...
			columns : [ {
				...
				formatter : function(value, row, index)
				{
					...
					if (Helper.basic.hasPermission('oem:order:list'))
					{
		...
		$("#bootTable").on('load-success.bs.table column-switch.bs.table page-change.bs.table search.bs.table', function()
		{
			...
			if ($(".glyphicon-th").next().html() == '')
			{
				...
				if (!Helper.basic.hasPermission('oem:order:export'))
				{
	...
	function order_edit(id)
	{
		...
		var url = Helper.basePath + '/oem/order/edit/' + id;
		var title = "代工订单";
	...
	function order_view(id)
	{
		...
		var url = Helper.basePath + '/oem/order/view/' + id;
		var title = "代工订单";
		admin_tab($("<a _href='"+url+"' data-title='"+title+"' />"));
	}
	// 删除
	function order_del(obj, id)
	{
		Helper.message.confirm('确认要删除吗？', function(index)
		{
			$.ajax({
				cache : true,
				type : "POST",
				url : Helper.basePath + '/oem/order/del/' + id,
				async : false,
				dataType : "json",
	...
	function create()
	{
		var url = Helper.basePath + '/oem/order/create';
		var title = "代工订单";
		admin_tab($("<a _href='"+url+"' data-title='"+title+"' />"));
	}
</script>
