...
<script type="text/javascript">
	...
	// 当页面渲染完毕后 ，重新算一遍数据，以免多批次退货时税额计算错误
	function trigger()
	{
	...
	function form_submit()
	{
		...
		Helper.request({
			url : Helper.basePath + "/oem/reconcil/save",
		...
		<div class="page-border">
			...
			<div class="cl">
				<div class="iframe-top">
					<sys:nav struct="代工管理-代工对账-创建"></sys:nav>
				</div>
				<div class="top_nav">
					<shiro:hasPermission name="oem:reconcil:create">
						<button class="nav_btn table_nav_btn" id="btn_save">保存</button>
					</shiro:hasPermission>
					<shiro:hasManyPermissions name="oem:reconcil:create,oem:reconcil:audit">
					...
					<div class="form-wrap">
						<div class="cl form_content">
							<dl class="cl row-dl">
								...
								<input type="hidden" name="originCompanyId" id="originCompanyId" value="${customer.originCompanyId }">
							...
							<dl class="cl row-dl">
								...
								<dd class="row-dd">
									...
									<span class="ui-combo-wrap">
										<phtml:list items="${fns:basicList2('SETTLEMENTCLASS')}" valueProperty="id" defaultValue="" defaultOption="请选择"
										 name="settlementClassId" textProperty="name" cssClass="nput-txt input-txt_1 hy_select2" selected="${customer.settlementClassId }"></phtml:list>
									</span>
								</dd>
								<dd class="row-dd">
							...
							<tbody>
								<c:forEach var="detail" items="${order.detailList}" varStatus="status">
									<tr name="tr">
										...
										<td class="td-manage">
											<c:if test="${detail.sourceBillType=='OEM_ED' }">送货单</c:if>
											<c:if test="${detail.sourceBillType=='OEM_ER' }">退货单</c:if>
										</td>
										<td>
										...
										<td>
											...
											<input type="hidden" name="detailList.oemOrderBillId" readonly="readonly" value="${detail.oemOrderBillId }" />
										</td>
										<td>
											<input class="tab_input bg_color <c:if test="${detail.sourceBillType=='OEM_ER' }">constraint_decimal_positive</c:if> <c:if test="${detail.sourceBillType=='OEM_ED' }">constraint_decimal_negative</c:if>" 
											 type="text" name="detailList.qty" value="<c:if test="${detail.sourceBillType=='OEM_ER' }">-</c:if>${detail.qty }" />
										...
										<td>
											<input class="tab_input bg_color" type="text" name="detailList.money" value="<c:if test="${detail.sourceBillType=='OEM_ER' }">-</c:if>${detail.money }" />
											<input type="hidden" name="detailList.saveMoney" value="<c:if test="${detail.sourceBillType=='OEM_ER' }">-</c:if>${detail.money }" />
										</td>
										<td>
											<input class="tab_input" type="text" name="detailList.tax" readonly="readonly" value="<c:if test="${detail.sourceBillType=='OEM_ER' }">-</c:if>${detail.tax}" />
										</td>
										...
										<td style="display: none">
											<input class="tab_input" type="text" name="detailList.noTaxMoney" readonly="readonly" value="<c:if test="${detail.sourceBillType=='OEM_ER' }">-</c:if>${detail.noTaxMoney }" />
										</td>
										<td>
