...
<title>创建发外到货单</title>
<script type="text/javascript" src="${ctxStatic }/site/outsource/arrive/create.js?v=${v }"></script>
</head>
<body>
		...
		<div class="page-border">
			...
			<div class="cl">
				<div class="iframe-top">
					<sys:nav struct="发外管理-发外到货-创建"></sys:nav>
				</div>
				<div class="top_nav">
					<shiro:hasPermission name="outsource:arrive:create">
						<button class="nav_btn table_nav_btn" id="btn_save">保存</button>
					</shiro:hasPermission>
					<shiro:hasManyPermissions name="outsource:arrive:create,outsource:arrive:audit">
						...
						<div class="cl form_content">
							<dl class="cl row-dl">
								...
								<dd class="row-dd">
									...
									<span class="ui-combo-wrap">
										<input type="text" class="input-txt input-txt_1 Wdate" onFocus="WdatePicker({lang:'zh-cn',minDate: '%y-%M-%d'})" name="deliveryTime" id="deliveryTime" value="${order.deliveryTime }" />
									</span>
								</dd>
							...
							<dl class="cl row-dl">
								...
								<dd class="row-dd">
									...
									<span class="ui-combo-wrap" class="form_text">
										<phtml:list items="${fns:basicList2('SETTLEMENTCLASS')}" valueProperty="id" textProperty="name" name="settlementClassId" cssClass="input-txt input-txt_1 hy_select2" selected="${supplier.settlementClassId }" />
									</span>
								</dd>
								<dd class="row-dd">
							...
							<tbody>
								<c:forEach var="detail" items="${order.detailList}" varStatus="status">
									<tr name="tr">
										...
										<input name="detailList.outSourceBillNo" type="hidden" value="${detail.outSourceBillNo }" />
										<td>${status.count}</td>
										<td class="td-manage">
										...
										<td class="td-manage" style="display: none;">
											<input class="tab_input" type="text" name="detailList.taxRatePercent" readonly="readonly" value="${detail.taxRatePercent }" />
										</td>
										<td class="td-manage" style="display: none;">
