...
{
	/* 判断当前用户是否有权限查看金额 */
	var hasPermission = Helper.basic.hasPermission('purch:reconcil_detail:money');
	// 查询，刷新table
	...
	$("#detailList").bootstrapTable({
		url : Helper.basePath + "/purch/reconcil/ajaxDetailList",
		method : "post",
		...
		cookieIdTable : "print_purch_reconcilDetial_master",// 必须制定唯一的表格cookieID
		...
		columns : [ {
			...
			formatter : function(value, row, index)
			{
				return (row.money - row.paymentMoney) < 0 ? 0 : (Number(row.money).subtr(Number(row.paymentMoney))).tomoney();
			}
		}, {
			field : 'tax',
			title : '税额',
		...
		onLoadSuccess : function()
		{
			...
			if ($(".glyphicon-th").next().html() == '')
			{
				...
				if (!Helper.basic.hasPermission('purch:reconcil_detail:export'))
				{
...
function queryParams(params)
{
	...
	params['orderBillNo'] = $("#orderBillNo").val();
