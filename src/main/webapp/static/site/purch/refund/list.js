$(function()
{
	...
	$("#detailList").bootstrapTable({
		url : Helper.basePath + "/purch/refund/ajaxList",
		method : "post",
		...
		cookieIdTable : "print_purch_refund_master",// 必须制定唯一的表格cookieID
		...
		columns : [ {
			...
			formatter : function(value, row, index)
			{
				...
				var operator = "";
				if (Helper.basic.hasPermission('purch:refund:list'))
				{

					operator += '<span class="table_operator"><a title="查看" href="javascript:;" onclick="refund_view(' + row.id + ')" style="padding: 0 8px; color: green"><i class="fa fa-search"></i></a></span>';
				...
				if (Helper.isNotEmpty(row.isCheck) && row.isCheck == 'NO')
				{
					...
					if (Helper.basic.hasPermission('purch:refund:list'))
					{
						operator += '<span class="table_operator"><a title="查看" href="javascript:;" onclick="refund_view(' + row.id + ')" style="padding: 0 8px; color: green"><i class="fa fa-search"></i></a>';

					}
					if (Helper.basic.hasPermission('purch:refund:edit'))
					{
						operator += '<a title="编辑" href="javascript:;" onclick="refund_edit(' + row.id + ')" style="padding: 0 8px; color: green"><i class="fa fa-pencil"></i></a>';
					}
					if (Helper.basic.hasPermission('purch:refund:del'))
					{
						operator += '<a title="删除" href="javascript:;" onclick="refund_del(' + row.id + ')" style="padding: 0 8px; color: red"><i class="fa fa-trash-o"></i></a></span>';
					}
				}
				return operator;
			}
		...
		onLoadSuccess : function()
		{
			...
			if ($(".glyphicon-th").next().html() == '')
			{
				...
				if (!Helper.basic.hasPermission('purch:refund:export'))
				{
...
function add()
{
	var url = Helper.basePath + '/purch/refund/create';
	var title = "采购退货";
	admin_tab($("<a _href='" + url + "' data-title='" + title + "' />"));
}
function refund_edit(id)
{
	if (!Helper.isNotEmpty(id))
	{
		return;
	}
	var url = Helper.basePath + '/purch/refund/edit/' + id;
	var title = "采购退货";
	admin_tab($("<a _href='" + url + "' data-title='" + title + "' />"));
}
...
function refund_del(id)
{
	Helper.message.confirm('确认要删除吗？', function(index)
	{
		$.ajax({
			cache : true,
			type : "POST",
			url : Helper.basePath + '/purch/refund/delete/' + id,
			async : false,
			dataType : "json",
			...
			success : function(data)
			{
				if (data.success)
				{
					...
					// window.location.href=Helper.basePath + '/purch/refund/list';
				} else
				{
					layer.alert("失败");
				}
