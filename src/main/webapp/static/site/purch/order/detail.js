...
{
	/* 判断当前用户是否有权限查看金额 */
	var hasPermission = Helper.basic.hasPermission('purch:order_detail:money');
	// 查询，刷新table
	...
	$("#detailList").bootstrapTable({
		url : Helper.basePath + "/purch/order/ajaxDetailList",
		method : "post",
		...
		cookieIdTable : "print_purch_orderDetial_master",// 必须制定唯一的表格cookieID
		...
		columns : [ {
			...
			formatter : function(value, row, index)
			{
				return (row.qty - row.storageQty) < 0 ? 0 : Number(row.qty).subtr(Number(row.storageQty));
			}
		}, {
			field : 'valuationQty',
			title : '计价数量',
		...
		onLoadSuccess : function(data)
		{
			...
			if ($(".glyphicon-th").next().html() == '')
			{
				...
				if (!Helper.basic.hasPermission('purch:order_detail:export'))
				{
...
function queryParams(params)
{
	...
	params['sourceBillNo'] = $("#workNo").val();
