$(function()
{
	...
	$("#detailList").bootstrapTable({
		url : Helper.basePath + "/purch/order/ajaxList",
		method : "post",
		...
		cookieIdTable : "print_purch_order_master",// 必须制定唯一的表格cookieID
		...
		columns : [ {
			...
			formatter : function(value, row, index)
			{
				...
				var operator = "";
				if (Helper.basic.hasPermission('purch:order:list'))
				{
					operator += '<span class="table_operator"><a title="查看" href="javascript:;" onclick="order_view(' + row.id + ')" style="padding: 0 8px; color: green"><i class="fa fa-search"></i></a></span>';
				}
				if (Helper.isNotEmpty(row.isCheck) && row.isCheck == 'NO')
				{
					...
					if (Helper.basic.hasPermission('purch:order:list'))
					{
						operator += '<span class="table_operator"><a title="查看" href="javascript:;" onclick="order_view(' + row.id + ')" style="padding: 0 8px; color: green"><i class="fa fa-search"></i></a>';
					}
					if (Helper.basic.hasPermission('purch:order:edit'))
					{
						operator += '<a title="编辑" href="javascript:;" onclick="order_edit(' + row.id + ')" style="padding: 0 8px; color: green"><i class="fa fa-pencil"></i></a>';
					}
					if (Helper.basic.hasPermission('purch:order:del'))
					{
						operator += '<a title="删除" href="javascript:;" onclick="order_del(' + row.id + ')" style="padding: 0 8px; color: red"><i class="fa fa-trash-o"></i></a></span>';
					}
		...
		onLoadSuccess : function()
		{
			...
			if ($(".glyphicon-th").next().html() == '')
			{
				...
				if (!Helper.basic.hasPermission('purch:order:export'))
				{
...
function add()
{
	var url = Helper.basePath + '/purch/order/create';
	var title = "采购订单";
...
function order_edit(id)
{
	...
	var url = Helper.basePath + '/purch/order/edit/' + id;
	var title = "采购订单";
	admin_tab($("<a _href='" + url + "' data-title='" + title + "' />"));
}
...
function order_del(id)
{
	Helper.message.confirm('确认要删除吗？', function(index)
	{
		$.ajax({
			cache : true,
			type : "POST",
			url : Helper.basePath + '/purch/order/delete/' + id,
			async : false,
			dataType : "json",
			...
			success : function(data)
			{
				if (data.success)
				{
					...
					// window.location.href=Helper.basePath + '/purch/order/list';
				} else
				{
					layer.alert("失败");
				}
