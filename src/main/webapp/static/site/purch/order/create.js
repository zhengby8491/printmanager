...
$(function()
{
	...
	// 采购时间 初始化
	$("#purchTime").val(new Date().format('yyyy-MM-dd'));
	// 删除一行数据
	...
	$("table input[name='detailList.percent']").each(function()
	{
		var isCopy = $("body").data("iscopy");
		if (isCopy == false)
		{
			var percent = Helper.basic.info('TAXRATE', $("#taxRateId").find("option:selected").val()).percent;
			$(this).val(percent);
		}
		$(this).parent().parent().find("input[name='detailList.deliveryTime']").val(new Date().format('yyyy-MM-dd'));
	});
...
{
	Helper.request({
		url : Helper.basePath + "/purch/order/save",
	...
	$.each(rows, function()
	{
		...
		$("#detailList").find("thead tr th").each(function()
		{
			...
			case 'price':
				...
				break;
			case 'deliveryTime':
				_TR.append('<td><input name="detailList.deliveryTime" onFocus="WdatePicker({lang:\'zh-cn\',minDate: \'%y-%M-%d\' })" readonly="readonly" class="tab_input bg_color" type="text" value="' + deliveryTime + '"/></td>');
				break;
			case 'sourceBillType':
