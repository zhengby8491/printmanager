...
$(function()
{
	...
	$("#save").click(function()
	{
		...
		Helper.request({
			url : Helper.basePath + "/basic/unitConvert/update",
...
// 将实际代码变量运算逻辑转为中文运算逻辑
function formulaChangeBack()
{
	var str = $("#formula").val();
	str = str.replace(new RegExp("length", "g"), "材料长");
	str = str.replace(new RegExp("weight", "g"), "克重");
	str = str.replace(new RegExp("width", "g"), "材料宽");
	str = str.replace(new RegExp("height", "g"), "材料高");
	str = str.replace(new RegExp("qty", "g"), "原单位数量");
	$("#formula_text").val(str);
}
