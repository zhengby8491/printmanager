$(function()
{
	...
	$(document).on("click", "tr td a[name='btn_del']", function()
	{
		...
		if ($(this).parent().parent().find("input[name*='.isDefault']").attr("name") == "addressList.isDefault")
		{
			...
			resetSequenceNum($("#supplier_address"));
		} else
		{
			$("input[name='payer_isDefault']").eq(0).trigger("click");
			resetSequenceNum($("#supplier_payer"));
		}
	});
	...
	{
		linkTr().appendTo("#supplier_address");
		resetSequenceNum($("#supplier_address"));
		var isDefault = false;
	...
	{
		payerTr().appendTo("#supplier_payer");
		resetSequenceNum($("#supplier_payer"));
		var isDefault = false;
...
function form_submit()
{
	...
	Helper.request({
		url : Helper.basePath + "/basic/supplier/update",
		...
		success : function(data)
		{
			if (data.success)
			{
				...
				parent.location.href = Helper.basePath + '/basic/supplier/list/';
			} else
			{
