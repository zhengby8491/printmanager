...
$(function()
{
	...
	{
		Helper.popup.show('选择客户', Helper.basePath + '/quick/customer_select?multiple=true', '850', '480');
	});
	...
	$("#delBtn").click(function()
	{
		...
		Helper.post(Helper.basePath + '/basic/product/delImg?fileName=' + encodeURI(fileName), function(data)
		{
			if (data.success)
			{
...
function appendTr(tableId, rows)
{
	...
	$.each(rows, function()
	{
		...
		$("#" + tableId).find("thead tr th").each(function()
		{
			...
			{
			case 'seq':
				_TR.append('<td><input name="customerList.id" class="tab_input" readonly="true" type="hidden" value="' + _THIS.id + '"/></td>');
				break;
			case 'code':
