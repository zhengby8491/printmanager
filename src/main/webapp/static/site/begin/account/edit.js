...
$(function()
{
	...
	{
		location.href = Helper.basePath + '/begin/account/view/' + $("input[name='id']").val();
	});
	setRowIndex();
	// 保存
...
{
	Helper.request({
		url : Helper.basePath + "/begin/account/update",
...
function getCallInfo_accountArray(rows)
{
	$.each(rows, function()
	{
		...
		var str = '<tr><td name="rowIndex"></td>' + '<td class="td-manage"><a title="删除行" href="javascript:void(0)" class="row_delete"><i class="delete fa fa-trash-o"></i></a></td>' + '<td><input name="detailList.bankNo" class="tab_input" type="text" readonly="readonly"value="' + this.bankNo + '"/></td>' + '<td><input name="detailList.accountId" class="tab_input id_' + this.id + '" type="hidden" value="' + this.id + '"/><input name="detailList.branchName" class="tab_input" onmouseover="this.title=this.value" readonly="readonly" type="text" value="' + this.branchName + '"/></td>' + '<td><input name="detailList.currencyType" class="tab_input" type="hidden" value="' + this.currencyType + '"/>'
