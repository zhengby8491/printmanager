...
$(function()
{
	...
	{
		var url = Helper.basePath + '/begin/supplier/list';
		var title = "供应商期初列表";
		admin_tab($("<a _href='" + url + "' data-title='" + title + "' />"));
	});
	$("#detailList").bootstrapTable({
		url : Helper.basePath + "/begin/supplier/viewAjax/" + $("#id").val(),// 不需要查询
		method : "post",
		...
		cookieIdTable : "print_begin_supplier_view",// 必须制定唯一的表格cookieID
	...
	{
		window.location.href = Helper.basePath + '/begin/supplier/edit/' + $("#id").val();
	});
	$("#btn_audit").click(function()
	{
		$.ajax({
			...
			type : "POST",
			dataType : "json",
			url : Helper.basePath + '/begin/supplier/audit/' + $("#id").val(),
			async : false,
	...
	$("#btn_audit_cancel").click(function()
	{
		$.ajax({
			...
			type : "POST",
			dataType : "json",
			url : Helper.basePath + '/begin/supplier/auditCancel/' + $("#id").val(),
			async : false,
			error : function(request)
			{
				layer.alert("Connection error");
	...
	$("#btn_del").click(function()
	{
		Helper.message.confirm('确认要删除吗？', function(index)
		{
			$.ajax({
				...
				type : "POST",
				dataType : "json",
				url : Helper.basePath + '/begin/supplier/delete/' + $("#id").val(),
				async : false,
				error : function(request)
				{
					// console.log(request);
				...
				success : function(data)
				{
					...
					if (data.success)
					{
						...
						/* window.location.href=Helper.basePath + '/begin/supplier/list'; */
					} else
					{
						layer.alert("失败");
					}
