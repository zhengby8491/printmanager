...
$(function()
{
	...
	{
		var url = Helper.basePath + '/begin/material/list';
		var title = "材料期初列表";
		admin_tab($("<a _href='" + url + "' data-title='" + title + "' />"));
	});
	$("#detailList").bootstrapTable({
		url : Helper.basePath + "/begin/material/viewAjax/" + $("#id").val(),// 不需要查询
		method : "post",
	...
	{
		window.location.href = Helper.basePath + '/begin/material/edit/' + $("#id").val();
	});
	$("#btn_audit").click(function()
	{
		$.ajax({
			...
			type : "POST",
			dataType : "json",
			url : Helper.basePath + '/begin/material/audit/' + $("#id").val(),
			async : false,
	...
	$("#btn_audit_cancel").click(function()
	{
		$.ajax({
			...
			type : "POST",
			dataType : "json",
			url : Helper.basePath + '/begin/material/auditCancel/' + $("#id").val(),
			async : false,
			error : function(request)
			{
				layer.alert("Connection error");
	...
	$("#btn_del").click(function()
	{
		Helper.message.confirm('确认要删除吗？', function(index)
		{
			$.ajax({
				...
				type : "POST",
				dataType : "json",
				url : Helper.basePath + '/begin/material/delete/' + $("#id").val(),
				async : false,
				error : function(request)
				{
					// console.log(request);
				...
				success : function(data)
				{
					if (data.success)
					{
						closeTabAndJump("材料期初列表");
						/* window.location.href=Helper.basePath + '/begin/material/list'; */
					} else
					{
						layer.alert("失败");
					}
