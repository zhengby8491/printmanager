...
$(function()
{
	if ($("#warehouseId option").length <= 0)
	{
		alert("所有仓库已建立期初");
...
{
	Helper.request({
		url : Helper.basePath + "/begin/product/save/",
...
function getCallInfo_productArray(rows)
{
	...
	$.each(rows, function()
	{
		...
		var str = '<tr><td name="rowIndex">1</td>' 
				...
				+ '<td><input name="detailList.productId" class="tab_input id_' + this.id + '" type="hidden" value="' + this.id 
				+ '"/><input readonly="readonly" name="detailList.productName" class="tab_input" type="text" value="' + this.name + '"/></td>'
				+ '<td><input readonly="readonly" name="detailList.specifications" class="tab_input" type="text" value="' + (this.specifications || "") + '"/></td>'
				+ '<td><input readonly="readonly" name="detailList.unitName" class="tab_input" type="text" value="' + Helper.basic.info('UNIT', this.unitId).name + '"/><input name="detailList.unit" type="hidden" value="' + this.unitId + '"/></td>' 
				+ '<td><input name="detailList.qty" class="tab_input constraint_negative bg_color" type="text" value=""/></td>' 
				+ '<td><input name="detailList.price" class="tab_input constraint_decimal bg_color" type="text" value="' + (this.salePrice || 0) + '"/></td>' 
				+ '<td><input readonly="readonly" name="detailList.money" class="tab_input" type="text" value=""/></td>'
