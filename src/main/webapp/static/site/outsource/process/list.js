$(function()
{
	...
	$("#bootTable").bootstrapTable({
		url : Helper.basePath + "/outsource/process/ajaxList",
		method : "post",
		...
		cookieIdTable : "print_outsource_process_master",// 必须制定唯一的表格cookieID
		...
		columns : [ {
			 ...
			 * , { field : 'progressStatusText', title : '进度状态', width : 80 }
			 */, {
			field : 'billNo',
			title : '加工单号',
			width : 140,
			...
			formatter : function(value, row, index)
			{
				...
				var operator = "";
				if (Helper.basic.hasPermission('outsource:process:list'))
				{
					operator += '<span class="table_operator"><a title="查看" href="javascript:;" onclick="process_view(' + row.id + ')" style="padding: 0 8px; color: green"><i class="fa fa-search"></i></a></span>';
				}
				if (Helper.isNotEmpty(row.isCheck) && row.isCheck == 'NO')
				{
					...
					if (Helper.basic.hasPermission('outsource:process:list'))
					{
						operator += '<span class="table_operator"><a title="查看" href="javascript:;" onclick="process_view(' + row.id + ')" style="padding: 0 8px; color: green"><i class="fa fa-search"></i></a>';
					}
					if (Helper.basic.hasPermission('outsource:process:edit'))
					{
						operator += '<a title="编辑" href="javascript:;" onclick="process_edit(' + row.id + ')" style="padding: 0 8px; color: green"><i class="fa fa-pencil"></i></a>';
					}
					if (Helper.basic.hasPermission('outsource:process:del'))
					{
						operator += '<a title="删除" href="javascript:;" onclick="process_del(this,' + row.id + ')" style="padding: 0 8px; color: red"><i class="fa fa-trash-o"></i></a></span>';
					}
				}
				return operator;
			}
	...
	$("#bootTable").on('load-success.bs.table column-switch.bs.table page-change.bs.table search.bs.table', function()
	{
		...
		if ($(".glyphicon-th").next().html() == '')
		{
			...
			if (!Helper.basic.hasPermission('outsource:process:export'))
			{
...
function process_edit(id)
{
	...
	var url = Helper.basePath + '/outsource/process/edit/' + id;
	var title = "发外加工单";
	admin_tab($("<a _href='" + url + "' data-title='" + title + "' />"))
}

/* 删除 */
function process_del(obj, id)
{
	Helper.message.confirm('确认要删除吗？', function(index)
	{
		Helper.post(Helper.basePath + '/outsource/process/del/' + id, function(data)
		{
			if (data.success)
			{
