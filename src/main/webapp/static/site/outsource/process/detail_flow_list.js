...
{
	/* 判断当前用户是否有权限查看金额 */
	var hasPermission = Helper.basic.hasPermission('outsource:flow:money');
	/* 更多 */
	...
	$("#bootTable").bootstrapTable({
		...
		columns : [ {
			...
			formatter : function(value, row, index)
			{
				var progress = [ '加工', '到货', '对账', '付款' ];
				if (row.qty > 0)
				{
					progress.splice(0, 1, '<span class="c-green">加工</span>');
				}

				if (row.arriveQty > 0)
				{
					progress.splice(1, 1, '<span class="c-green">到货</span>');
				}
				...
				if (row.payment > 0)
				{
					progress.splice(3, 1, '<span class="c-green">付款</span>');
				}
			...
			formatter : function(value, row, index)
			{
				return (row.qty - value) < 0 ? 0 : (row.qty - value).roundFixed(2);
			}
		}, {
			field : 'reconcilQty',
			title : '对账数量',
			...
			formatter : function(value, row, index)
			{
				return (row.money - value) < 0 ? 0 : (row.money - value).roundFixed(2);
			}
		}, {
			field : 'workBillNo',
			title : '生产单号',
		...
		onLoadSuccess : function()
		{
			...
			if ($(".glyphicon-th").next().html() == '')
			{
				...
				if (!Helper.basic.hasPermission('outsource:flow:export'))
				{
