...
$(function()
{
	...
	{
		location.href = Helper.basePath + '/outsource/return/view/' + $("#order_id").val();
	});
	/* 选择供应商 */
	...
	$("#supplier_quick_select").click(function()
	{
		...
		Helper.popup.show('来源外发到货', Helper.basePath + '/outsource/return/quick_select?supplierId=' + $(this).val(), '800', '490');
	});
	 ...
	 * }); //纠正仓库名称 $("table select[name='detailList.warehouse']").each(function() {
	 * 
...
function batchEditTaxRate()
{
	...
	$("table select[name='detailList.taxRate']").val(taxRateId);
	// $("input[name='detailList.percent']").val(Helper.basic.info("TAXRATE",taxRateId).percent);
	$("table select[name='detailList.taxRate']").trigger("change");
}
function form_submit()
{
	...
	Helper.request({
		url : Helper.basePath + "/outsource/return/update",
		data : paramObj,
		...
		$("#detailList").find("thead tr th").each(
				function()
				{
					...
					case 'money':
						...
						break;
					case 'taxRate':
						_TR.append('<td><input name="detailList.taxRateName" readonly="readonly" class="tab_input" type="text" value="' + Helper.basic.info('TAXRATE', _THIS.taxRateId).name + '"/>');
						break;
					case 'taxRatePercent':
						_TR.append('<td style="display: none;"><input type="text" name="detailList.taxRatePercent" readonly="readonly" value="' + _THIS.taxRatePercent + '"/></td>');
						break;
					case 'noTaxPrice':
					...
					case 'partName':
						...
						break;
					case 'memo':
						_TR.append('<td><input class="tab_input bg_color memo" type="text" name="detailList.memo" value="' + (_THIS.memo || '') + '"/></td>' + '<input name="detailList.procedureId" type="hidden" value="' + (_THIS.procedureId || '') + '">' + '<input name="detailList.procedureCode" type="hidden" value="' + (_THIS.procedureCode || '') + '">' + '<input name="detailList.procedureType" type="hidden" value="' + (_THIS.procedureType || '') + '">' + '<input name="detailList.taxRateId" type="hidden" value="' + _THIS.taxRateId + '">' + '<input name="detailList.sourceBillType" type="hidden" value="' + _THIS.master.billType + '"/>' + '<input name="detailList.sourceId" type="hidden" value="' + _THIS.master.id + '"/>'
								+ '<input name="detailList.sourceDetailId" type="hidden" value="' + _THIS.id + '"/>' + '<input name="detailList.productId" type="hidden" value="' + (_THIS.productId || '') + '"/>' + '<input name="detailList.outSourceBillNo" type="hidden" value="' + _THIS.outSourceBillNo + '"/>' + '<input name="detailList.type" type="hidden" value="' + _THIS.type + '"/>' + '<input name="detailList.productType" type="hidden" value="' + (_THIS.productType || "") + '" />' + '<input name="detailList.workProcedureType" type="hidden" value="' + (_THIS.workProcedureType || "") + '"/>' + '</td></tr>');
						break;
					}
				});
