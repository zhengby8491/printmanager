...
$(function()
{
	...
	$("table select[name='detailList.taxRateId']").trigger("change");// 出发税率明细改变事件
...
function form_submit()
{
	...
	Helper.request({
		url : Helper.basePath + "/outsource/reconcil/save",
...
function calcTaxRate(obj)
{
	...
	// 获取单价对象并格式化
	var price_dom = $(obj).parent().parent().find("input[name='detailList.price']");
	var price_value = Number(price_dom.val()).toFixed(4);// 单价
	// 不含税单价计算
