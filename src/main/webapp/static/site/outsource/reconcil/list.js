$(function()
{
	...
	$("#bootTable").bootstrapTable({
		url : Helper.basePath + "/outsource/reconcil/ajaxList",
		method : "post",
		...
		cookieIdTable : "print_outsource_reconcil_master",// 必须制定唯一的表格cookieID
		...
		columns : [ {
			...
			formatter : function(value, row, index)
			{
				...
				var operator = "";
				if (Helper.basic.hasPermission('outsource:reconcil:list'))
				{
				...
				if (Helper.isNotEmpty(row.isCheck) && row.isCheck == 'NO')
				{
					...
					if (Helper.basic.hasPermission('outsource:reconcil:edit'))
					{
					...
					if (Helper.basic.hasPermission('outsource:reconcil:del'))
					{
						operator += '<a title="删除" href="javascript:;" onclick="reconcil_del(this,' + row.id + ')" style="padding: 0 8px; color: red"><i class="fa fa-trash-o"></i></a></span>';
					}
				}
				return operator;
			}
	...
	$("#bootTable").on('load-success.bs.table column-switch.bs.table page-change.bs.table search.bs.table', function()
	{
		...
		if ($(".glyphicon-th").next().html() == '')
		{
			...
			;
			if (!Helper.basic.hasPermission('outsource:reconcil:export'))
			{
...
function reconcil_edit(id)
{
	...
	var url = Helper.basePath + '/outsource/reconcil/edit/' + id;
	var title = "发外对账单";
	admin_tab($("<a _href='" + url + "' data-title='" + title + "' />"))
}

/* 删除 */
function reconcil_del(obj, id)
{
	Helper.message.confirm('确认要删除吗？', function(index)
	{
		Helper.post(Helper.basePath + '/outsource/reconcil/del/' + id, function(data)
		{
			if (data.success)
			{
