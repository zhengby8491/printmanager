...
{
	/* 判断当前用户是否有权限查看金额 */
	var hasPermission = Helper.basic.hasPermission('outsource:reconcil_detail:money');
	/* 更多 */
	...
	$("#bootTable").bootstrapTable({
		url : Helper.basePath + "/outsource/reconcil/ajaxDetailList",
		method : "post",
		...
		cookieIdTable : "print_outsource_reconcil_detail",// 必须制定唯一的表格cookieID
		...
		columns : [ {
			...
			formatter : function(value, row, index)
			{
				return Number(row.money).subtr(Number(value));
			}
		}, {
			field : 'tax',
			title : '税额',
		...
		onLoadSuccess : function(data)
		{
			...
			if ($(".glyphicon-th").next().html() == '')
			{
				...
				if (!Helper.basic.hasPermission('outsource:reconcil_detail:export'))
				{
		...
		onDblClickRow : function(row)
		{
			...
			var url = Helper.basePath + '/outsource/reconcil/view/' + row.master.id;
			var title = "发外对账单";
