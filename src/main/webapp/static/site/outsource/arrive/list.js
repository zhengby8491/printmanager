$(function()
{
	...
	{
		Helper.popup.show('选择供应商', Helper.basePath + '/quick/supplier_select?multiple=false', '800', '500');
	});
	/* 查询审核状态 */
	...
	$("#bootTable").bootstrapTable({
		url : Helper.basePath + "/outsource/arrive/ajaxList",
		method : "post",
		...
		cookieIdTable : "print_outsource_arrive_master",// 必须制定唯一的表格cookieID
		...
		columns : [ {
			...
			formatter : function(value, row, index)
			{
				...
				if (Helper.basic.hasPermission('outsource:arrive:list'))
				{
					operator += '<a title="查看" href="javascript:;" onclick="arrive_view(' + row.id + ')" style="padding: 0 8px; color: green"><i class="fa fa-search"></i></a>';

				}
				if (Helper.isNotEmpty(row.isCheck) && row.isCheck == 'NO')
				{

					if (Helper.basic.hasPermission('outsource:arrive:edit'))
					{
						operator += '<a title="编辑" href="javascript:;" onclick="arrive_edit(' + row.id + ')" style="padding: 0 8px; color: green"><i class="fa fa-pencil"></i></a>';
					}
					if (Helper.basic.hasPermission('outsource:arrive:del'))
					{
						operator += '<a title="删除" href="javascript:;" onclick="arrive_del(this,' + row.id + ')" style="padding: 0 8px; color: red"><i class="fa fa-trash-o"></i></a>';
					}
				}
				operator += '</span>';
				return operator;
	...
	$("#bootTable").on('load-success.bs.table column-switch.bs.table page-change.bs.table search.bs.table', function()
	{
		...
		if ($(".glyphicon-th").next().html() == '')
		{
			...
			;
			if (!Helper.basic.hasPermission('outsource:arrive:export'))
			{
...
/* 编辑 */
function arrive_edit(id)
{
	if (!Helper.isNotEmpty(id))
	{
		return;
	}
	var url = Helper.basePath + '/outsource/arrive/edit/' + id;
	var title = "发外到货单";
	admin_tab($("<a _href='" + url + "' data-title='" + title + "' />"))
}

/* 删除 */
function arrive_del(obj, id)
{
	Helper.message.confirm('确认要删除吗？', function(index)
	{
		Helper.post(Helper.basePath + '/outsource/arrive/del/' + id, function(data)
		{
			if (data.success)
			{
