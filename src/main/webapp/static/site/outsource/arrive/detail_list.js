$(function()
{
	var hasPermission = Helper.basic.hasPermission('outsource:arrive_detail:money');
	/* 更多 */
	...
	$("#bootTable").bootstrapTable({
		url : Helper.basePath + "/outsource/arrive/ajaxDetailList",
		method : "post",
		...
		cookieIdTable : "print_outsource_arrive_detail",// 必须制定唯一的表格cookieID
		...
		columns : [ {
			...
			formatter : function(value, row, index)
			{
				return (row.qty - row.reconcilQty).roundFixed(2);
			}
		}, {
			field : 'price',
			title : '单价',
		...
		onLoadSuccess : function(data)
		{
			...
			if ($(".glyphicon-th").next().html() == '')
			{
				...
				if (!Helper.basic.hasPermission('outsource:arrive_detail:export'))
				{
		...
		onDblClickRow : function(row)
		{
			...
			var url = Helper.basePath + '/outsource/arrive/view/' + row.master.id;
			var title = "发外到货单";
		...
		onRefresh : function()
		{
			$('#bootTable').bootstrapTable('resetView');
		}
	});
