$(function()
{
	...
	$("#btn_transmit").click(function()
	{
		...
		if (Helper.isNotEmpty(rows))
		{
			...
			{// 判断是否统一供应商
				...
				var url = Helper.basePath + '/outsource/arrive/create?supplierId=' + supplierId + '&' + _paramStr;
				var title = "发外到货单";
	...
	$("#bootTable").bootstrapTable({
		url : Helper.basePath + "/outsource/transmit/to_arrive_list",
		method : "post",
		...
		cookieIdTable : "print_outsource_transmit_arrive",// 必须制定唯一的表格cookieID
		...
		columns : [ {
			...
			formatter : function(value, row, index)
			{
				return Number(row.qty).subtr(Number(row.arriveQty));
			}
		}, {
			field : 'memo',
			title : '备注',
	...
	$("#bootTable").on('load-success.bs.table column-switch.bs.table page-change.bs.table search.bs.table', function()
	{
		...
		if ($(".glyphicon-th").next().html() == '')
		{
			...
			if (!Helper.basic.hasPermission('transmit:to_arrive:export'))
			{
