...
$(function()
{
	...
	// 监听批量修改税收按钮，解决列表选项值不一致onchange问题
	/*
	 * $('#rateId').on("select2:closing", function(e) { batchEditTaxRate(); });
	 */
...
function form_submit()
{
	...
	Helper.request({
		url : Helper.basePath + "/sale/deliver/save",
		data : $("#form_payment").formToJson(),
		success : function(result)
		{
			...
			{
				Helper.message.warn('保存失败!' + result.message);
			}
			$("#btn_save").removeAttr("disabled");
			$("#btn_save_audit").removeAttr("disabled");
		},
...
function calculation()
{
	$("table tbody tr").each(function()
	{
		...
		var noTaxPrice_dom = $(this).find("td input[name='detailList.noTaxPrice']");

		// 计算金额
		var money_dom = $(this).find("td input[name='detailList.money']");

		// 不含税金额计算
		var noTaxMoney_dom = $(this).find("td input[name='detailList.noTaxMoney']");
