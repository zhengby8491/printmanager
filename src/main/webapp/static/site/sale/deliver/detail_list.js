...
{
	/* 判断当前用户是否有权限查看金额 */
	var hasPermission = Helper.basic.hasPermission('sale:deliver_detail:money');
	/* 选择客户 */
	...
	$("#bootTable").bootstrapTable({
		url : Helper.basePath + "/sale/deliver/ajaxDetailList",
		method : "post",
		...
		cookieIdTable : "print_sale_deliver_detail",// 必须制定唯一的表格cookieID
		...
		onDblClickRow : function(row)
		{
			...
			var url = Helper.basePath + '/sale/deliver/view/' + row.master.id;
			var title = "销售送货";
	...
	$("#bootTable").on('load-success.bs.table column-switch.bs.table page-change.bs.table search.bs.table', function()
	{
		...
		if ($(".glyphicon-th").next().html() == '')
		{
			...
			if (!hasPermission)
			{
				...
				$("div[title='列'] ul[role=menu]").find("input[value=24]").parent().parent().remove();
			}
			if (!Helper.basic.hasPermission('sale:deliver_detail:export'))
			{
...
function queryParams(params)
{
	...
	params['saleOrderBillNo'] = $("#saleOrderBillNo").val().trim();
	return params;
}
// ajax结果
