$(function()
{
	...
	$("#bootTable").bootstrapTable({
		url : Helper.basePath + "/sale/reconcil/ajaxList",
		method : "post",
...

		{
			...
			formatter : function(value, row, index)
			{
				...
				if (Helper.basic.hasPermission('sale:reconcil:list'))
				{
					operator += '<a title="查看" href="javascript:;" onclick="reconcil_view(' + row.id + ')" style="padding: 0 8px; color: green"><i class="fa fa-search"></i></a>';
				}
				if (Helper.isNotEmpty(row.isCheck) && row.isCheck == 'NO')
				{
					if (Helper.basic.hasPermission('sale:reconcil:edit'))
					{
					...
					if (Helper.basic.hasPermission('sale:reconcil:del'))
					{
						operator += '<a title="删除" href="javascript:;" onclick="reconcil_del(this,' + row.id + ')" style="padding: 0 8px; color: red"><i class="fa fa-trash-o"></i></a>';
					}
				}
				operator += '</span>';
				return operator;
	...
	$("#bootTable").on('load-success.bs.table column-switch.bs.table page-change.bs.table search.bs.table', function()
	{
		...
		if ($(".glyphicon-th").next().html() == '')
		{
			...
			if (!Helper.basic.hasPermission('sale:reconcil:export'))
			{
...
function reconcil_edit(id)
{
	...
	var url = Helper.basePath + '/sale/reconcil/edit/' + id;
	var title = "销售对账";
...
function reconcil_del(obj, id)
{
	...
	{
		Helper.post(Helper.basePath + '/sale/reconcil/del/' + id, function(data)
		{
			if (data.success)
			{
