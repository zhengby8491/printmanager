...
{
	/* 判断当前用户是否有权限查看金额 */
	var hasPermission = Helper.basic.hasPermission('sale:reconcil_detail:money');
	/* 选择客户 */
	...
	$("#bootTable").bootstrapTable({
		url : Helper.basePath + "/sale/reconcil/ajaxDetailList",
		method : "post",
		...
		cookieIdTable : "print_sale_reconcil_detail",// 必须制定唯一的表格cookieID
		...
		columns : [ {
			...
			formatter : function(value, row, index)
			{
				return row.money - row.receiveMoney;
			}
		}, {
			field : 'tax',
			title : '税额',
		...
		onDblClickRow : function(row)
		{
			...
			var url = Helper.basePath + '/sale/reconcil/view/' + row.master.id;
			var title = "销售对账";
	...
	$("#bootTable").on('load-success.bs.table column-switch.bs.table page-change.bs.table search.bs.table', function()
	{
		...
		if ($(".glyphicon-th").next().html() == '')
		{
			...
			if (!Helper.basic.hasPermission('sale:reconcil_detail:export'))
			{
...
function queryParams(params)
{
	...
	params['saleOrderBillNo'] = $("#saleOrderBillNo").val();
