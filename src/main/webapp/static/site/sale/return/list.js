$(function()
{
	...
	/* 选择客户 */
	/*
	 * $("#customer_quick_select").click(function() { Helper.popup.show('选择客户',
	 * Helper.basePath + '/quick/customer_select?multiple=false', '900', '490');
	 * });
	 */
	/* 查询审核状态 */
	...
	$("#bootTable").bootstrapTable({
		url : Helper.basePath + "/sale/return/ajaxList",
		method : "post",
		...
		columns : [ {
			...
			formatter : function(value, row, index)
			{
				...
				if (Helper.basic.hasPermission('sale:return:list'))
				{
	...
	$("#bootTable").on('load-success.bs.table column-switch.bs.table page-change.bs.table search.bs.table', function()
	{
		...
		if ($(".glyphicon-th").next().html() == '')
		{
			...
			if (!Helper.basic.hasPermission('sale:return:export'))
			{
...
function return_edit(id)
{
	...
	var url = Helper.basePath + '/sale/return/edit/' + id;
	var title = "销售退货";
...
function return_del(obj, id)
{
	...
	{
		Helper.post(Helper.basePath + '/sale/return/del/' + id, function(data)
		{
			if (data.success)
			{
...
function toCreate()
{
	var url = Helper.basePath + '/sale/return/create/';
	var title = "销售退货";
	admin_tab($("<a _href='" + url + "' data-title='" + title + "' />"));
}