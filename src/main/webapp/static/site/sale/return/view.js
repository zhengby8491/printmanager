$(function()
{
	...
	$("#detailList").bootstrapTable({
		url : Helper.basePath + "/sale/return/viewAjax/" + saleReturnId,
		method : "post",
		...
		cookieIdTable : "print_sale_return_master",// 必须制定唯一的表格cookieID
	...
	{
		var url = Helper.basePath + '/sale/return/list';
		var title = "销售退货列表";
	...
	{
		var url = Helper.basePath + '/sale/return/edit/' + $("#id").val();
		var title = "销售退货";
		admin_tab($("<a _href='" + url + "' data-title='" + title + "' />"));
	});
	// 审核
	...
	{
		var order_id = $("#id").val();
		Helper.post(Helper.basePath + '/sale/return/check/' + order_id, function(data)
		{
			if (data.success)
			{
				Helper.message.suc(data.message);
				location.href = Helper.basePath + "/sale/return/view/" + order_id;
			} else
			{
	...
	{
		var order_id = $("#id").val();
		Helper.post(Helper.basePath + '/sale/return/checkBack/' + order_id, function(data)
		{
			if (data.success)
			{
				if (data.obj == null)
	...
	{
		var order_id = $("#id").val();
		Helper.post(Helper.basePath + '/sale/return/complete', {
			"tableType" : "MASTER",
			"ids" : [ order_id ]
		}, function(data)
		{
	...
	{
		var order_id = $("#id").val();
		Helper.post(Helper.basePath + '/sale/return/complete_cancel', {
			"tableType" : "MASTER",
			"ids" : [ order_id ]
		}, function(data)
		{
	...
	$("#btn_del").on("click", function()
	{
		...
		{
			Helper.post(Helper.basePath + '/sale/return/del/' + $("#id").val(), function(data)
			{
				if (data.success)
				{
					Helper.message.suc(data.message);
					closeTabAndJump("销售退货列表");
					/* location.href=Helper.basePath+"/sale/return/list"; */
				} else
				{
					Helper.message.warn(data.message);
				}
	...
	// 打印模板加载
	$("#btn_print").loadTemplate('SALE_IR', '/sale/return/printAjax/' + saleReturnId);
...
function forceCheckBack(id)
{
	$.ajax({
		...
		type : "POST",
		dataType : "json",
		url : Helper.basePath + '/sale/return/forceCheckBack/' + id,
		async : false,
