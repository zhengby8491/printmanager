...
$(function()
{
	...
	{
		location.href = Helper.basePath + '/sale/return/view/' + $("#order_id").val();
	});
	/* 选择客户 */
	...
	$("#select_deliver").click(function()
	{
		...
		Helper.popup.show('选择退货源', Helper.basePath + '/sale/deliver/quick_select?multiple=true&rowIndex=1&customerId=' + _customerId, '800', '490');
	});
...
function form_submit()
{
	...
	Helper.request({
		url : Helper.basePath + "/sale/return/update",
	...
	$.each(rows, function()
	{
		...
		$("#" + tableId).find("thead tr th").each(function()
		{
			...
			case 'seq':
				...
				if (!hasPermission)
				{
					_TR.append('<td style="display:none;"><input class="tab_input" readonly="readonly" type="text" name="detailList.price" value="' + value + '"/>' + '<input class="tab_input" type="text" readonly="readonly" name="detailList.money" value="' + (_THIS.money - _THIS.returnMoney - _THIS.reconcilMoney || 0) + '"/>' + '<input type="hidden" name="detailList.saveMoney" value="' + (_THIS.money - _THIS.returnMoney - _THIS.reconcilMoney || 0) + '"/>' + '<input class="tab_input" type="text" name="detailList.tax" readonly="readonly" value="' + value + '"/>')
				}
				break;
			case 'operator':
			...
			case 'warehouseId':
				...
				break;
			case 'price':
				_TR.append('<td><input class="tab_input" readonly="readonly" type="text" name="detailList.price" value="' + value + '"/></td>');
				break;
			case 'money':
