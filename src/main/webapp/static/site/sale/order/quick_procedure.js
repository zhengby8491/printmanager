...
$(function()
{
	...
	{
		$(this).val(Number($(this).val()).tomoney());
	});
	// 初始化产品生产数量
	...
	{
		$(this).siblings('input').attr("id", "machineName");
		Helper.popup.show('选择机台信息', Helper.basePath + '/quick/machine_select', '900', '490');
	})
	...
	$("#add_part").click(function()
	{
		...
		// 重新打开【工序材料】时，显示之前保存得数据
		if (_initReopen == false)
		{
			_reopenAndShow();
		}
		// 添加部件信息
	...
	function createPart()
	{
		...
		if (part_product_map.size() > 0)
		{
			...
			$.each(part_product_map.values(), function(i, obj)
			{
				...
				part_product_table.append("<tr><td><i title='删除' class='fa fa-trash-o'></i></td>" + "<td><input name='productList.productId' type='hidden' value='" + obj.id + "'/>" + "<input name='productList.productName' onmouseover='this.title=this.value' class='tab_input' readonly='readonly' type='text' value='" + obj.name + "'/></td>" + pageNum_td + "</tr>");
			});
		}
		// 设置部件数量
	...
	var _initReopen = false;
	function _reopenAndShow()
	{
		// console.log(parent._currentSelectProduct);
		// console.log(parent._allProduces)
		if (parent._currentSelectProduct)
		{
			var oldData = parent._allProduces[parent._currentSelectProduct.id];
			if (oldData)
			{
				// 成品
				$(oldData.pack).each(function(i, pack)
				{
					...
					{
						var newMaterial = {};
						newMaterial.oldId = material.id || null;
					...
					append_material(newMaterialList, true);
			...
			{
				// 部件
				$(oldData.partList).each(function(i, part)
				{
					...
					if (part_product_map.size() > 0)
					{
						// 用于更新
						var productIdStr = "";
						if (part.productList && part.productList.length == 1)
						{
							// productIdStr = "<input name='productList.id' type='hidden'
							// value='"+part.productList[0].id+"'/>";
						}
						var pageNum_td = "";
						if (isBook)
						{
						...
						{
							part_num = part_num.add(Number(obj.produceQty));
							part_product_table.append("<tr><td><i title='删除' class='fa fa-trash-o'></i></td>" + "<td>" + productIdStr + "<input name='productList.productId' type='hidden' value='" + obj.id + "'/>" + "<input name='productList.productName' onmouseover='this.title=this.value' class='tab_input' readonly='readonly' type='text' value='" + obj.name + "'/></td>" + pageNum_td + "</tr>");
						});
					}
					// 设置部件数量
					...
					if (part.id)
					{
						new_part_clone.find("input[name='partList.id']").val(part.id); // 部件id（用于更新）
					}
	...
	function _reopenAndShowProcedure(new_part_clone, procedureList)
	{
		$(procedureList).each(
				function(i, procedure)
				{
					...
					procedure_line.append("<div class='cl pri_item mar_r'><i class='arrow_r fa fa-long-arrow-right'></i> <span class='show_fw'> <label class='hy_hide radio_item'><input type='checkbox' title='发外' " + isOutSource_check + " />发外</label><span class='removeRequierBt'>X</span><label class='hy_hide radio_item2'>要求</label><input type='hidden' name='procedure_id' value='" + procedure_id + "'/><input type='hidden' name='procedure_code' value='" + procedure_code + "'/><input type='hidden' name='procedure_name' value='" + procedure_name + "'/><input type='hidden' name='procedure_procedureType' value='" + procedure_procedureType + "'/><input type='hidden' name='procedure_procedureClassId' value='"
							+ procedure_procedureClassId + "'/> <input type='hidden' name='procedure_isOutSource' value='" + procedure_isOutSource + "'/> <span class='ct' data-memo='" + procedure_memo + "'>" + procedure_name + "</span>" + isOutSourceText + "</span> <i title='删除' class='del_ico fa fa-close'></i> </div>");
				});
	}
	...
	$(document).on(
			...
			function()
			{
				...
				{
					procedure_line.append("<div class='cl pri_item mar_r'><i class='arrow_r fa fa-long-arrow-right'></i> <span class='show_fw'> <label class='hy_hide radio_item'><input type='checkbox' title='发外' " + isOutSource_check + " />发外</label><span class='removeRequierBt'>X</span><label class='hy_hide radio_item2'>要求</label><input type='hidden' name='procedure_id' value='" + procedure_id + "'/><input type='hidden' name='procedure_code' value='" + procedure_code + "'/><input type='hidden' name='procedure_name' value='" + procedure_name + "'/><input type='hidden' name='procedure_procedureType' value='" + procedure_procedureType + "'/><input type='hidden' name='procedure_procedureClassId' value='"
							+ procedure_procedureClassId + "'/> <input type='hidden' name='procedure_isOutSource' value='" + procedure_isOutSource + "'/> <span class='ct'>" + procedure_name + "</span>" + isOutSourceText + "</span> <i title='删除' class='del_ico fa fa-close'></i> </div>");
...
function count_part_impressionNum(partObj)
{
	...
	partObj.find(".work_table tbody input[name='partList.impressionNum']").val(_part_impressionNum.ceil());
...
function count_part_lossRate(partObj)
{
	...
	var _lossRate = Number(_lossQty).div(_impressionNum).mul(100).toFixed(4);
	if (isBook)
	{
		var _stickerlossQty = partObj.find(".work_table tbody input[name='partList.stickerlossQty']").val();
		var _stickersPostedNum = partObj.find(".work_table tbody input[name='partList.stickersPostedNum']").val();
...
function count_part_stickersNum(partObj)
{
	...
	{
		// 正反、单面： P数/(拼版数*2)
		partObj.find("input[name='partList.stickersNum']").val(0);
...
function count_part_materialNum(materialObj)
{
	...
	var _temp_materialObj_qty = totalImpressionNum.div(Number(materialObj.find("input[name='materialSplitQty']").val())).ceil();
...
{
	var _temp_sum_produceQty = Number($("#product_sum_produceQty").val());
	var _temp_materialObj_qty = _temp_sum_produceQty.div(Number(materialObj.find("input[name='materialSplitQty']").val())).ceil();
	materialObj.find("input[name='materialQty']").val(_temp_materialObj_qty);
}
...
// 添加材料
function append_material(rows, isProcedureFinished)
{
	$.each(rows, function(i, row)
	{
		...
		// 解决成品工艺-材料规格向上弹出来
		if (isProcedureFinished === true || curr_material_div.parents("#pack_div").length > 0)
		{
			clone_Item.find("div.td_box").css("top", "-200px");
		}

		// 用于更新
		if (row.oldId)
		{
			clone_Item.find("input[name='id']").val(row.oldId);
		}
...
function form_submit()
{
	...
	$("#partList_div .fold_table").each(function(index, item)
	{
		...
		$(item).find(".material-class").each(function(material_index, material_item)
		{
			...
			// part_list[index].materialList[material_index].splitQty =
			// $(material_item).find("input[name='materialSplitQty']").val();
			// part_list[index].materialList[material_index].qty =
			// $(material_item).find("input[name='materialQty']").val();
		...
		$(item).find(".procedure-line-class .pri_item").each(function(procedure_index, procedure_item)
		{
			...
			{// 印刷工序，取总印张数
				part_procedure_outputQty = part_list[index].totalImpressionNum;
			} else if ($(procedure_item).find("input[name='procedure_procedureType']").val() == "AFTER")
			{// 印后工序，取印张正数
				part_procedure_outputQty = part_list[index].impressionNum;
			}
	...
	$("#pack_div .material-class").each(function(index, item)
	{
		...
		// pack.materialList[index].splitQty =
		// $(item).find("input[name='materialSplitQty']").val();
		// pack.materialList[index].qty =
		// $(item).find("input[name='materialQty']").val();
	...
	parent.updateProcedure(parent._currentSelectProduct.id, request_work_json);
	Helper.popup.close();
}
...
$(function()
{
	...
	if (parent._currentSelectProduct)
	{
		getCallInfo_productArray([ parent._currentSelectProduct ]);
	} else
	{
		$("#btn_save").hide();
	}
