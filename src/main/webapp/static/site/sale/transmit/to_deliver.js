$(function()
{
		...
		if (Helper.isNotEmpty(rows))
		{
			...
			{// 判断是否统一供应商
				...
				$(rows).each(function(index, item)
				{
					if (item.id)
					{
						_paramStr += "&idsBillNos=" + item.master.billNo + "_" + item.id;
					}
				});
	...
	$("#bootTable").bootstrapTable({
		url : Helper.basePath + "/sale/transmit/to_deliver_list",
		method : "post",
...
function queryParams(params)
{
	...
	params['createTimeMin'] = $("#createTime1").val().trim();
	params['createTimeMax'] = $("#createTime2").val().trim();
