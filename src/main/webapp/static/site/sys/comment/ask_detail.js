var UEditor;
    $(function()
    {
        ...
        $("#detail_return").click(function(){
            var url=Helper.basePath + "/sys/comment/list";
    		var title="留言版";
    ...
    function save()
    {
        Helper.request({
            ...
            data: {id:$("#id").val(),reply:UEditor.getContent(),isManagerReplay:"YES"},
            ...
            {
                if(data.success) {
                    var url=Helper.basePath + "/sys/comment/ask_detail/"+data.obj.id;
          		    var title="服务支持";
          		    admin_tab($("<a _href='"+url+"' data-title='"+title+"' />"))
                }else {
                	Helper.message.warn("服务器繁忙");
