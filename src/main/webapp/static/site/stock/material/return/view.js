$(function()
{
	...
	$("#detailList").bootstrapTable({
		...
		cookieIdTable : "print_stockmaterial_return_master_view",// 必须制定唯一的表格cookieID
		columns : [ {
			field : 'id',
			title : '序号',
			width : 40,
	...
	{
		window.location.href = Helper.basePath + '/stockmaterial/return/edit/' + $(this).val();
	});
	$("#btn_back").click(function()
	{
		var url = Helper.basePath + '/stockmaterial/return/list';
		var title = "生产退料列表";
	...
	$("#btn_audit").click(function()
	{
		$.ajax({
			...
			type : "POST",
			dataType : "json",
			url : Helper.basePath + '/stockmaterial/return/check/' + $(this).val(),
			async : false,
	...
	$("#btn_audit_cancel").click(function()
	{
		...
		$.ajax({
			...
			type : "POST",
			dataType : "json",
			url : Helper.basePath + '/stockmaterial/return/checkBack/' + $id,
			async : false,
			error : function(request)
			{
				layer.alert("Connection error");
	...
	$("#btn_del").click(function()
	{
		...
		Helper.message.confirm('确认要删除吗？', function(index)
		{
			$.ajax({
				...
				type : "POST",
				dataType : "json",
				url : Helper.basePath + '/stockmaterial/return/delete/' + this_.val(),
				async : false,
				error : function(request)
				{
					// console.log(request);
				...
				success : function(data)
				{
					...
					if (data.success)
					{
						closeTabAndJump("生产退料列表");
						/*
						 * window.location.href=Helper.basePath +
						 * '/stockmaterial/return/list';
						 */
					} else
					{
						layer.alert("失败");
	...
	// 打印模板加载
	$("#btn_print").loadTemplate('STOCK_RM', '/stockmaterial/return/printAjax/' + $("#orderId").val());
...
function forceCheckBack(id)
{
	$.ajax({
		...
		type : "POST",
		dataType : "json",
		url : Helper.basePath + '/stockmaterial/return/forceCheckBack/' + id,
		async : false,
		error : function(request)
		{
			layer.alert("Connection error");
