...
{
	/* 判断当前用户是否有权限查看金额 */
	var hasPermission = Helper.basic.hasPermission('stock:materialReturn_detail:money');
	/* 选择客户 */
	...
	$("#bootTable").bootstrapTable({
		url : Helper.basePath + "/stockmaterial/return/ajaxDetailList",
		method : "post",
		...
		cookieIdTable : "print_stockmaterial_return_detail",// 必须制定唯一的表格cookieID
		...
		columns : [ {
			...
			formatter : function(value, row, index)
			{
				if (row.master.returnTime)
				{
					return new Date(row.master.returnTime).format("yyyy-MM-dd");
				}
			}
		}, {
			field : 'billNo',
		...
		onDblClickRow : function(row)
		{
			...
			var url = Helper.basePath + '/stockmaterial/return/view/' + row.master.id;
			var title = "生产退料";
