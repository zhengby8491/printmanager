...
{
	/* 判断当前用户是否有权限查看金额 */
	var hasPermission = Helper.basic.hasPermission('stock:materialSupplement_detail:money');
	/* 选择客户 */
	$("#customer_quick_select").click(function()
	{
		Helper.popup.show('选择客户', Helper.basePath + '/quick/customer_select?multiple=false', '980', '500');
	});
	/* 选择材料 */
	...
	$("#bootTable").bootstrapTable({
		url : Helper.basePath + "/stockmaterial/supplement/ajaxDetailList",
		method : "post",
		...
		cookieIdTable : "print_stockmaterial_supplement_detail",// 必须制定唯一的表格cookieID
		...
		columns : [ {
			...
			formatter : function(value, row, index)
			{
				if (row.master.supplementTime)
				{
					return new Date(row.master.supplementTime).format("yyyy-MM-dd");
				}
			}
		}, {
			field : 'billNo',
			...
			field : 'workBillTypeText',
			title : '源单类型',
			width : 80,
			formatter : function(value, row, index)
			{
				return row.master.workBillTypeText;
			}
		}, {
			field : 'master.workBillNo',
			title : '生产单号',
			width : 120,
			formatter : function(value, row, index)
			{
				return billNoTransToUrl(row.master.workBillNo);
			}
		}, {
			field : 'code',
			title : '材料编号',
		...
		onDblClickRow : function(row)
		{
			...
			var url = Helper.basePath + '/stockmaterial/supplement/view/' + row.master.id;
			var title = "生产补料";
