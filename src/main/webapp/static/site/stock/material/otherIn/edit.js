...
$(function()
{
	...
	{
		location.href = Helper.basePath + '/stockmaterial/otherin/view/' + $("input[name='id']").val();
	});
	...
	$.each(rows, function()
	{
		...
		$("#detailList").find("thead tr th").each(function()
		{
			...
			case 'price':
				...
				break;
			case 'memo':
				_TR.append('<td><input name="detailList.memo" class="tab_input bg_color memo" onmouseover="this.title=this.value" type="text" /></td></tr>');
				break;
			}
		});
...
function form_submit()
{
	...
	Helper.request({
		url : Helper.basePath + "/stockmaterial/otherin/update",
		data : data,
