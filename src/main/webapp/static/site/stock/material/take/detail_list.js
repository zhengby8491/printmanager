...
{
	/* 判断当前用户是否有权限查看金额 */
	var hasPermission = Helper.basic.hasPermission('stock:materialTake_detail:money');
	/* 选择客户 */
	...
	$("#bootTable").bootstrapTable({
		url : Helper.basePath + "/stockmaterial/take/ajaxDetailList",
		method : "post",
		...
		cookieIdTable : "print_stockmaterial_take_detail",// 必须制定唯一的表格cookieID
		...
		columns : [ {
			...
			formatter : function(value, row, index)
			{
				if (row.master.takeTime)
				{
					return new Date(row.master.takeTime).format("yyyy-MM-dd");
				}
			}
		}, {
			field : 'billNo',
		...
		onDblClickRow : function(row)
		{
			...
			var url = Helper.basePath + '/stockmaterial/take/view/' + row.master.id;
			var title = "生产领料";
...
function queryParams(params)
{
	...
	params['workBillNo'] = $("#sourceBillNo").val();
