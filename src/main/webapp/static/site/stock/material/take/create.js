...
$(function()
{
	...
	// 日期初始化
	$("#takeTime").val(new Date().format('yyyy-MM-dd'));
...
function form_submit()
{
	...
	Helper.request({
		url : Helper.basePath + "/stockmaterial/take/save",
		data : $("#form_take").formToJson(),
		success : function(result)
		{
			if (!result.success)
			{
				return Helper.message.warn('保存失败!' + result.message);
			}
			var data = result.obj;
			if (data.isSuccess)
			{
...
function getCallInfo_material_stock(content)
{
	for (var i = 0; i < content.length; i++)
	{
		...
		var str = '<tr><td name="rowIndex"></td>' + '<td class="td-manage">' + '<i class="row_delete fa fa-trash-o"></i></td>' + '<td><input name="detailList.code" class="tab_input" type="text" readonly="readonly" value="' + obj.material.code + '"/></td>' + '<td><input name="detailList.materialName" class="tab_input" type="text" readonly="readonly" value="' + obj.material.name + '"/></td>' + '<td><input name="detailList.specifications" class="tab_input" type="text"  readonly="readonly" value="' + (obj.specifications || "") + '"/></td>' + '<td><input name="detailList.stockUnitName" class="tab_input" type="text" readonly="readonly" value="' + Helper.basic.info('UNIT', obj.material.stockUnitId).name + '"/>' + '</td>'
				+ '<td><input name="detailList.qty" class="tab_input constraint_decimal bg_color" type="text" value="' + obj.qty + '"/></td>' + '<td><input name="detailList.sourceQty" class="tab_input" type="text" readonly="readonly" value=""/></td>' + (hasPermission ? '<td>' : '<td style="display:none;">') + '<input name="detailList.price" class="tab_input constraint_decimal bg_color" type="text" value="' + (obj.price || 0) + '"/></td>' + (hasPermission ? '<td>' : '<td style="display:none;">') + '<input name="detailList.money" class="tab_input" readonly="readonly" type="text" value=""/></td>' + '<td><input name="" class="tab_input" type="text" readonly="readonly" value=""/></td>'
				+ '<td><input name="" class="tab_input" type="text" readonly="readonly" value=""/></td>' + '<td><input name="" class="tab_input" type="text" readonly="readonly" value=""/></td>' + '<td><input name="detailList.memo" class="tab_input bg_color memo" type="text" value=""/>' + '<input name="detailList.weight" type="text" readonly="hidden" value="' + obj.material.weight + '"/>' + '<input name="detailList.valuationPrice" type="text" readonly="hidden" value="' + (obj.valuationPrice || 0) + '"/>' + '<input name="detailList.materialId" type="hidden"  value="' + obj.materialId + '"/>' + '<input name="detailList.valuationUnitId" type="hidden" value="' + obj.material.valuationUnitId + '"/>'
				+ '<input name="detailList.materialClassId" type="hidden" value="' + obj.material.materialClassId + '"/>' + '<input name="detailList.stockMaterialId" type="hidden" value="' + obj.id + '"/>' + '<input name="valuationUnitAccuracy" type="hidden" value="' + Helper.basic.info('UNIT', obj.material.valuationUnitId).accuracy + '"/>' + '<input name="detailList.stockUnitId" type="hidden" value="' + obj.material.stockUnitId + '"/>' + '<input name="detailList.valuationQty" type="hidden" value=""/>' + '<input name="detailList.valuationUnitName" type="hidden" value="' + Helper.basic.info('UNIT', obj.material.valuationUnitId).name + '"/>' + '<input name="stockUnitAccuracy" type="hidden" value="'
				+ Helper.basic.info('UNIT', obj.material.stockUnitId).accuracy + '"/>' + '</td></tr>';
		$("#tbody").append(str);
	}
	setRowIndex();
	$("input[name='detailList.qty']").trigger("input");
