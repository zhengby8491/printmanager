$(function()
{
	...
	$("#bootTable").bootstrapTable({
		url : Helper.basePath + "/stockmaterial/adjust/ajaxList",
		method : "post",
		...
		cookieIdTable : "print_stockmaterial_adjust_master",// 必须制定唯一的表格cookieID
...
{
	var url = Helper.basePath + '/stockmaterial/adjust/edit/' + id;
	var title = "材料库存调整";
...
{
	var url = Helper.basePath + '/stockmaterial/adjust/view/' + id;
	var title = "材料库存调整";
	admin_tab($("<a _href='" + url + "' data-title='" + title + "' />"));
}
/* 删除 */
function adjust_del(obj, id)
{
	...
	{
		Helper.post(Helper.basePath + '/stockmaterial/adjust/delete/' + id, function(data)
		{
			if (data.success)
			{
...
function toCreate()
{
	var url = Helper.basePath + '/stockmaterial/adjust/create';
	var title = "材料库存调整";
	admin_tab($("<a _href='" + url + "' data-title='" + title + "' />"));
}