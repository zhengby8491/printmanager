$(function()
{
	...
	$("#detailList").bootstrapTable({
		...
		cookieIdTable : "print_stockmaterial_inventory_master_view",// 必须制定唯一的表格cookieID
		columns : [ {
			field : 'id',
			title : '序号',
			width : 40,
	...
	{
		window.location.href = Helper.basePath + '/stockmaterial/inventory/edit/' + $(this).val();
	});
	$("#btn_back").click(function()
	{
		var url = Helper.basePath + '/stockmaterial/inventory/list';
		var title = "材料库存盘点列表";
	...
	$("#btn_audit").click(function()
	{
		$.ajax({
			...
			type : "POST",
			dataType : "json",
			url : Helper.basePath + '/stockmaterial/inventory/check/' + $(this).val(),
			async : false,
	...
	$("#btn_del").click(function()
	{
		...
		Helper.message.confirm('确认要删除吗？', function(index)
		{
			$.ajax({
				...
				type : "POST",
				dataType : "json",
				url : Helper.basePath + '/stockmaterial/inventory/delete/' + this_.val(),
				async : false,
				error : function(request)
				{
					// console.log(request);
	...
	// 打印模板加载
	$("#btn_print").loadTemplate('STOCK_SMI', '/stockmaterial/inventory/printAjax/' + $("#orderId").val());
