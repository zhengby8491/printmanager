...
$(function()
{
	...
	{
		location.href = Helper.basePath + '/stockmaterial/inventory/view/' + $("input[name='id']").val();
	});
	setRowIndex();
	// 保存
...
{
	Helper.request({
		url : Helper.basePath + "/stockmaterial/inventory/update",
...
function getCallInfo_material_stock(content)
{
	for (var i = 0; i < content.length; i++)
	{
		...
		var str = '<tr><td name="rowIndex">1</td>' + '<td class="td-manage">' + '<button  class="row_delete"><i class="delete fa fa-trash-o"></i></button>' + '<td><input name="detailList.code" class="tab_input" type="text" readonly="readonly" value="' + obj.material.code + '"/></td>' + '<td><input name="detailList.materialName" class="tab_input" type="text" readonly="readonly" value="' + obj.material.name + '"/></td>' + '<td><input name="detailList.specifications" class="tab_input" readonly="true" type="text" value="' + (obj.specifications || "") + '"/></td>' + '<td><input name="detailList.weight" class="tab_input" type="text" value="' + obj.material.weight + '"/></td>'
				...
				+ '"/></td>' + (hasPermission ? '<td>' : '<td style="display:none;">') + '<input name="detailList.valuationPrice" class="tab_input constraint_decimal" type="text" readonly="readonly" value="' + obj.valuationPrice + '"/></td>' + (hasPermission ? '<td>' : '<td style="display:none;">') + '<input name="detailList.price" class="tab_input" type="text" readonly="readonly" value="' + obj.price + '"/></td>' + (hasPermission ? '<td>' : '<td style="display:none;">') + '<input name="detailList.money" class="tab_input" type="text" readonly="readonly" value=""/></td>' + '<td><input name="detailList.profitAndLossQty" class="tab_input" type="text" readonly="readonly" value="0"/></td>'
				+ (hasPermission ? '<td>' : '<td style="display:none;">') + '<input name="detailList.profitAndLossMoney" class="tab_input" type="text" readonly="readonly" value=""/></td>' + '<td><input name="detailList.inventoryTypeName" class="tab_input" type="text" readonly="readonly" value=""/></td>' + '<td><input name="detailList.memo" class="tab_input bg_color memo" onmouseover="this.title=this.value" type="text" value=""/></td>' + '<td style="display:none"><input name="detailList.materialId" class="tab_input" type="hidden" readonly="readonly" value="' + obj.materialId + '"/></td>' + '<td style="display:none"><input name="detailList.valuationUnitId" class="tab_input" type="hidden" readonly="readonly" value="'
				+ obj.material.valuationUnitId + '"/></td>' + '<td style="display:none"><input name="detailList.materialClassId" class="tab_input" type="hidden" readonly="readonly" value="' + obj.material.materialClassId + '"/></td>' + '<td style="display:none"><input name="detailList.stockMaterialId" class="tab_input id_' + obj.id + '" type="hidden" readonly="readonly" value="' + obj.id + '"/></td>' + '<td style="display:none"><input name="detailList.inventoryType" class="tab_input" type="hidden" readonly="readonly" value=""/></td>' + '<td style="display:none"><input name="valuationUnitAccuracy" class="tab_input" type="hidden" readonly="readonly" value="'
