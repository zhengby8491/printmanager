/* 判断当前用户是否有权限查看金额 */
	...
	$(function()
	{
		...
		{
			location.href = Helper.basePath + '/stockmaterial/transfer/view/' + $("input[name='id']").val();
		});
		setRowIndex();
		//保存
	...
	function form_submit()
	{
		...
		Helper.request({
			url : Helper.basePath + "/stockmaterial/transfer/update",
	...
	function getCallInfo_material_stock(content)
	{
		for (var i = 0; i < content.length; i++)
		{
			...
			var str = '<tr><td name="rowIndex">1</td>' + '<td class="td-manage">' + '<i class="delete_row fa fa-trash-o"></i>' + '<td><input name="detailList.code" class="tab_input id_'+obj.id+'" type="text" readonly="readonly" value="'+obj.material.code+'"/></td>' + '<td><input name="detailList.materialName" class="tab_input" type="text" readonly="readonly" value="'+obj.material.name+'"/></td>' + '<td><input name="detailList.specifications" class="tab_input" readonly="readonly" type="text" value="' + (obj.specifications || "") + '"/></td>' + '<td><input name="detailList.weight" class="tab_input" readonly="readonly" type="text" value="'+obj.material.weight+'"/></td>'
					...
					+ '<td style="display:none"><input name="detailList.stockQty" class="tab_input" type="hidden" value="'+obj.qty+'"/></td>' + '<td style="display:none"><input name="detailList.materialId" class="tab_input" type="hidden" readonly="readonly" value="'+obj.materialId+'"/></td>' + '<td style="display:none"><input name="detailList.valuationUnitId" class="tab_input" type="hidden" readonly="readonly" value="'+obj.material.valuationUnitId+'"/></td>' + '<td style="display:none"><input name="detailList.materialClassId" class="tab_input" type="hidden" readonly="readonly" value="'+obj.material.materialClassId+'"/></td>'
					+ '<td style="display:none"><input name="detailList.stockMaterialId" class="tab_input" type="hidden" readonly="readonly" value="'+obj.id+'"/></td>' + '<td style="display:none"><input name="valuationUnitAccuracy" class="tab_input" type="hidden" readonly="readonly" value="' + Helper.basic.info('UNIT', obj.material.valuationUnitId).accuracy + '"/></td>' + '<td style="display:none"><input name="stockUnitAccuracy" class="tab_input" type="hidden" readonly="readonly" value="' + Helper.basic.info('UNIT', obj.material.stockUnitId).accuracy + '"/></td>'
