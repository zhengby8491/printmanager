$(function()
{
	...
	$("#detailList").bootstrapTable({
		...
		cookieIdTable : "print_stockmaterial_transfer_master_view",// 必须制定唯一的表格cookieID
		columns : [ {
			field : 'id',
			title : '序号',
			width : 40,
	...
	{
		window.location.href = Helper.basePath + '/stockmaterial/transfer/edit/' + $(this).val();
	});
	$("#btn_back").click(function()
	{
		var url = Helper.basePath + '/stockmaterial/transfer/list';
		var title = "材料库存调拨列表";
	...
	$("#btn_audit").click(function()
	{
		...
		$.ajax({
			...
			type : "POST",
			dataType : "json",
			url : Helper.basePath + '/stockmaterial/transfer/check/' + $id,
			async : false,
	...
	$("#btn_audit_cancel").click(function()
	{
		...
		$.ajax({
			...
			type : "POST",
			dataType : "json",
			url : Helper.basePath + '/stockmaterial/transfer/checkBack/' + $id,
			async : false,
			error : function(request)
			{
				layer.alert("Connection error");
	...
	$("#btn_del").click(function()
	{
		...
		Helper.message.confirm('确认要删除吗？', function(index)
		{
			$.ajax({
				...
				type : "POST",
				dataType : "json",
				url : Helper.basePath + '/stockmaterial/transfer/delete/' + this_.val(),
				async : false,
				error : function(request)
				{
					// console.log(request);
	...
	// 打印模板加载
	$("#btn_print").loadTemplate('STOCK_SMT', '/stockmaterial/transfer/printAjax/' + $("#orderId").val());
...
function forceCheckBack(id)
{
	$.ajax({
		...
		type : "POST",
		dataType : "json",
		url : Helper.basePath + '/stockmaterial/transfer/forceCheckBack/' + id,
		async : false,
		error : function(request)
		{
			layer.alert("Connection error");
