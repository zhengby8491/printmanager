/* 判断当前用户是否有权限查看金额 */
	var hasPermission = Helper.basic.hasPermission('stock:productInventory:money');
		$(function(){
		  $("#btn_cancel").click(function(){
			...
			$("#product_quick_select").click(function(){		
				Helper.popup.show('选择产品库存', Helper.basePath
						+ '/stock/product/quick_select?warehouseId='+$(this).val() + '&isEmptyWare=no', '1030', '490');
			});
		...
		function form_submit()
		{
			  ...
			  Helper.request({   
			    url: Helper.basePath+"/stockproduct/inventory/save",  
			    data: data,
		...
		function getCallInfo_product_stock(content) {
			for (var i = 0; i < content.length; i++) {
				...
				var str='<tr><td name="rowIndex">1</td>'
							...
							+'<td><input name="detailList.code" class="tab_input" type="text" readonly="readonly" value="'+obj.product.code+'"/></td>'
							+'<td><input name="detailList.productName" class="tab_input" type="text" readonly="readonly" value="'+obj.product.name+'"/></td>'
							+'<td><input name="detailList.specifications" class="tab_input" readonly="readonly" type="text" value="'+(obj.product.specifications||"")+'"/></td>'
							+'<td><input name="detailList.unitName" class="tab_input" type="text" readonly="readonly" value="'+Helper.basic.info('UNIT',obj.product.unitId).name+'"/></td>'						
							+'<td><input name="detailList.qty" class="tab_input constraint_negative bg_color" type="text" value="'+obj.qty+'"/></td>'
							+'<td><input name="detailList.stockQty" class="tab_input" type="text" readonly="readonly" value="'+obj.qty+'"/></td>'
							...
							+'<td><input name="detailList.profitAndLossQty" class="tab_input" type="text" readonly="readonly" value="0"/></td>'
							+(hasPermission ?'<td>' : '<td style="display:none;">')+'<input name="detailList.profitAndLossMoney" class="tab_input" type="text" readonly="readonly" value=""/></td>'
							+'<td><input name="detailList.inventoryTypeName" class="tab_input" type="text" readonly="readonly" value=""/></td>'
							+'<td><input name="detailList.memo" class="tab_input bg_color memo" onmouseover="this.title=this.value" type="text" value=""/></td>'	
							+'<td style="display:none"><input name="detailList.productId" class="tab_input" type="hidden" readonly="readonly" value="'+obj.productId+'"/></td>'
							+'<td style="display:none"><input name="detailList.productClassId" class="tab_input" type="hidden" readonly="readonly" value="'+obj.product.productClassId+'"/></td>'
							+'<td style="display:none"><input name="detailList.stockProductId" class="tab_input id_'+obj.id+'" type="hidden" readonly="readonly" value="'+obj.id+'"/></td>'
							+'<td style="display:none"><input name="detailList.inventoryType" class="tab_input" type="hidden" readonly="readonly" value=""/></td>'
							+'<td style="display:none"><input name="detailList.unitId" class="tab_input" type="hidden" readonly="readonly" value="'+obj.product.unitId+'"/></td>'
							+'<td>'+ (obj.product.imgUrl==''?'':'<img class="pimg" src="'+ obj.product.imgUrl +'"/>')+'</td></tr>';
