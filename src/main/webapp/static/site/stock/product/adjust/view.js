$(function()
{
	...
	$("#detailList").bootstrapTable({
		...
		cookieIdTable : "print_stockproduct_adjust_master_view",// 必须制定唯一的表格cookieID
		columns : [ {
			field : 'id',
			title : '序号',
			width : 40,
	...
	{
		window.location.href = Helper.basePath + '/stockproduct/adjust/edit/' + $(this).val();
	});
	$("#btn_back").click(function()
	{
		var url = Helper.basePath + '/stockproduct/adjust/list';
		var title = "成品库存调整列表";
	...
	$("#btn_audit").click(function()
	{
		$.ajax({
			...
			type : "POST",
			dataType : "json",
			url : Helper.basePath + '/stockproduct/adjust/check/' + $(this).val(),
			async : false,
	...
	$("#btn_del").click(function()
	{
		...
		Helper.message.confirm('确认要删除吗？', function(index)
		{
			$.ajax({
				...
				type : "POST",
				dataType : "json",
				url : Helper.basePath + '/stockproduct/adjust/delete/' + this_.val(),
				async : false,
				error : function(request)
				{
					// console.log(request);
	...
	// 打印模板加载
	$("#btn_print").loadTemplate('STOCK_SPA', '/stockproduct/adjust/printAjax/' + $("#orderId").val());
