$(function()
{
	...
	$("#bootTable").bootstrapTable({
		url : Helper.basePath + "/stockproduct/adjust/ajaxList",
		method : "post",
		...
		cookieIdTable : "print_stockproduct_adjust_master",// 必须制定唯一的表格cookieID
		...
		columns : [ {
			...
			formatter : function(value, row, index)
			{
				...
				var operator = "";
				if (Helper.basic.hasPermission('stock:productAdjust:list'))
				{
...
{
	var url = Helper.basePath + '/stockproduct/adjust/edit/' + id;
	var title = "成品库存调整";
...
{
	var url = Helper.basePath + '/stockproduct/adjust/view/' + id;
	var title = "成品库存调整";
	admin_tab($("<a _href='" + url + "' data-title='" + title + "' />"));
}
...
function adjust_del(obj, id)
{
	...
	{
		Helper.post(Helper.basePath + '/stockproduct/adjust/delete/' + id, function(data)
		{
			if (data.success)
			{
...
function toCreate()
{
	var url = Helper.basePath + '/stockproduct/adjust/create';
	var title = "成品库存调整";
	admin_tab($("<a _href='" + url + "' data-title='" + title + "' />"));
}