...
{
	/* 判断当前用户是否有权限查看金额 */
	var hasPermission = Helper.basic.hasPermission('stock:productIn_detail:money');
	...
	$("#bootTable").bootstrapTable({
		url : Helper.basePath + "/stockproduct/in/ajaxDetailList",
		method : "post",
		...
		cookieIdTable : "print_stockproduct_in_detail",// 必须制定唯一的表格cookieID
		...
		columns : [ {
			...
			formatter : function(value, row, index)
			{
				if (row.master.inTime)
				{
					return new Date(row.master.inTime).format("yyyy-MM-dd");
				}
			}
		}, {
			field : 'billNo',
		...
		onLoadSuccess : function()
		{
			...
			if ($(".glyphicon-th").next().html() == '')
			{
				...
				if (!Helper.basic.hasPermission('stock:product_in:export'))
				{
		...
		onDblClickRow : function(row)
		{
			...
			var url = Helper.basePath + '/stockproduct/in/view/' + row.master.id;
			var title = "成品入库";
