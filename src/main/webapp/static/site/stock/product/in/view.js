...
{
	/* 判断当前用户是否有权限查看金额 */
	var hasPermission = Helper.basic.hasPermission('stock:productIn:money');
	...
	$("#detailList").bootstrapTable({
		...
		cookieIdTable : "print_stockproduct_in_master_view",// 必须制定唯一的表格cookieID
		columns : [ {
			field : 'id',
			title : '序号',
			width : 40,
	...
	{
		window.location.href = Helper.basePath + '/stockproduct/in/edit/' + $(this).val();
	});
	$("#btn_back").click(function()
	{
		var url = Helper.basePath + '/stockproduct/in/list';
		var title = "成品入库列表";
	...
	$("#btn_audit").click(function()
	{
		$.ajax({
			...
			type : "POST",
			dataType : "json",
			url : Helper.basePath + '/stockproduct/in/check/' + $(this).val(),
			async : false,
	...
	$("#btn_audit_cancel").click(function()
	{
		...
		$.ajax({
			...
			type : "POST",
			dataType : "json",
			url : Helper.basePath + '/stockproduct/in/checkBack/' + $id,
			async : false,
			error : function(request)
			{
				layer.alert("Connection error");
			...
			success : function(data)
			{
				if (data.success)
				{
					...
					} else
					{
						...
						{
							str = str + '编号:' + data.obj[i].product.name + '   目前库存数量' + data.obj[i].qty + '\n';
						}
	...
	$("#btn_del").click(function()
	{
		...
		Helper.message.confirm('确认要删除吗？', function(index)
		{
			$.ajax({
				...
				type : "POST",
				dataType : "json",
				url : Helper.basePath + '/stockproduct/in/delete/' + this_.val(),
				async : false,
				error : function(request)
				{
					// console.log(request);
				...
				success : function(data)
				{
					if (data.success)
					{
						...
						/* window.location.href=Helper.basePath + '/stockproduct/in/list'; */
					} else
					{
						layer.alert("失败");
					}
	...
	// 打印模板加载
	$("#btn_print").loadTemplate('STOCK_SPIN', '/stockproduct/in/printAjax/' + $("#inId").val());
...
function forceCheckBack(id)
{
	$.ajax({
		...
		type : "POST",
		dataType : "json",
		url : Helper.basePath + '/stockproduct/in/forceCheckBack/' + id,
		async : false,
		error : function(request)
		{
			layer.alert("Connection error");
