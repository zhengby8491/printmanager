$(function()
{
	...
	$("#detailList").bootstrapTable({
		...
		cookieIdTable : "print_stockproduct_otherin_master_view",// 必须制定唯一的表格cookieID
		columns : [ {
			field : 'id',
			title : '序号',
			width : 40,
	...
	{
		window.location.href = Helper.basePath + '/stockproduct/otherin/edit/' + $(this).val();
	});
	$("#btn_back").click(function()
	{
		var url = Helper.basePath + '/stockproduct/otherin/list';
		var title = "成品其它入库列表";
	...
	$("#btn_audit").click(function()
	{
		$.ajax({
			...
			type : "POST",
			dataType : "json",
			url : Helper.basePath + '/stockproduct/otherin/check/' + $(this).val(),
			async : false,
	...
	$("#btn_audit_cancel").click(function()
	{
		...
		$.ajax({
			...
			type : "POST",
			dataType : "json",
			url : Helper.basePath + '/stockproduct/otherin/checkBack/' + $id,
			async : false,
			error : function(request)
			{
				layer.alert("Connection error");
	...
	$("#btn_del").click(function()
	{
		...
		Helper.message.confirm('确认要删除吗？', function(index)
		{
			$.ajax({
				...
				type : "POST",
				dataType : "json",
				url : Helper.basePath + '/stockproduct/otherin/delete/' + this_.val(),
				async : false,
				error : function(request)
				{
					// console.log(request);
				...
				success : function(data)
				{
					...
					if (data.success)
					{
						closeTabAndJump("成品品其它入库列表");
						/*
						 * window.location.href=Helper.basePath +
						 * '/stockproduct/otherin/list';
						 */
					} else
					{
						layer.alert("失败");
	...
	// 打印模板加载
	$("#btn_print").loadTemplate('STOCK_SPOI', '/stockproduct/otherin/printAjax/' + $("#orderId").val());
...
function forceCheckBack(id)
{
	$.ajax({
		...
		type : "POST",
		dataType : "json",
		url : Helper.basePath + '/stockproduct/otherin/forceCheckBack/' + id,
		async : false,
		error : function(request)
		{
			layer.alert("Connection error");
