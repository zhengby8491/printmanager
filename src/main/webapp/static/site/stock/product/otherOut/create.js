...
function getCallInfo_product_stock(content)
{
	for (var i = 0; i < content.length; i++)
	{
		...
		var str = '<tr><td name="rowIndex">1</td>' + '<td class="td-manage">' + '<i class="delete_row fa fa-trash-o"></i>' + '<td><input name="detailList.code" class="tab_input id_' + obj.id + '" type="text" readonly="readonly" value="' + obj.product.code + '"/></td>' + '<td><input name="detailList.productName" class="tab_input" type="text" readonly="readonly" value="' + obj.product.name + '"/></td>' + '<td><input name="detailList.specifications" class="tab_input" readonly="readonly" type="text" value="' + (obj.product.specifications || "") + '"/></td>' + '<td><input name="detailList.unitName" class="tab_input" type="text" readonly="readonly" value="' + Helper.basic.info('UNIT', obj.product.unitId).name
				+ '"/></td>' + '<td><input name="detailList.qty" class="tab_input constraint_negative bg_color" type="text" value="' + obj.qty + '"/></td>' + (hasPermission ? '<td>' : '<td style="display:none;">') + '<input name="detailList.price" class="tab_input" type="text" readonly="readonly" value="' + obj.price + '"/></td>' + (hasPermission ? '<td>' : '<td style="display:none;">') + '<input name="detailList.money" class="tab_input" type="text" readonly="readonly" value="' + Number(obj.price).mul(obj.qty) + '"/></td>' + '<td><input name="detailList.memo" class="tab_input bg_color memo" onmouseover="this.title=this.value" type="text" value=""/></td>' + '<td>'
				+ (obj.product.imgUrl == '' ? '' : '<img class="pimg" src="' + obj.product.imgUrl + '"/>') + '</td>' + '<td style="display:none"><input name="detailList.productId" class="tab_input" type="hidden" readonly="readonly" value="' + obj.productId + '"/></td>' + '<td style="display:none"><input name="detailList.productClassId" class="tab_input" type="hidden" readonly="readonly" value="' + obj.product.productClassId + '"/></td>' + '<td style="display:none"><input name="detailList.stockProductId" class="tab_input" type="hidden" readonly="readonly" value="' + obj.id + '"/></td>' + '<td style="display:none"><input name="detailList.unitId" class="tab_input" type="hidden" readonly="readonly" value="'
				+ obj.product.unitId + '"/></td></tr>';
		$("#tbody").append(str);
	}
	setRowIndex();
	trigger();
...
function form_submit()
{
	...
	Helper.request({
		url : Helper.basePath + "/stockproduct/otherout/save",
		data : data,
