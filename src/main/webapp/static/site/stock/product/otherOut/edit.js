/* 判断当前用户是否有权限查看金额 */
	...
	$(function()
	{
		...
		{
			location.href = Helper.basePath + '/stockproduct/otherout/view/' + $("input[name='id']").val();
		});
	...
	function getCallInfo_product_stock(content)
	{
		...
		for (var i = 0; i < content.length; i++)
		{
			...
			var str = '<tr><td name="rowIndex">1</td>' + '<td class="td-manage">' + '<i class="delete_row fa fa-trash-o"></i>' 
					+ '<td><input name="detailList.code" class="tab_input id_'+obj.id+'" type="text" readonly="readonly" value="'+obj.product.code+'"/></td>'
					...
					+ (hasPermission ? '<td>' : '<td style="display:none;">') + '<input name="detailList.money" class="tab_input" type="text" readonly="readonly" value="' + Number(obj.price).mul(obj.qty) + '"/></td>' 
					+ '<td><input name="detailList.memo" class="tab_input bg_color memo" onmouseover="this.title=this.value" type="text" value=""/></td>' + '<td>'
					+ (obj.product.imgUrl == '' ? '' : '<img class="pimg" src="'+ obj.product.imgUrl +'"/>') + '</td>' 
					...
					+ '<td style="display:none"><input name="detailList.stockProductId" class="tab_input" type="hidden" readonly="readonly" value="'+obj.id+'"/></td>'
	...
	function form_submit()
	{
		...
		Helper.request({
			url : Helper.basePath + "/stockproduct/otherout/update",
			data : data,
				...
				if (data.isSuccess)
				{
					...
					if (data.returnObject != null && data.returnObject.length > 0)
					{
						...
						Helper.message.confirm(str, function(index)
						{
							...
							{
								location.href = Helper.basePath + '/stockproduct/otherout/view/' + data.returnValue.id;
							});
						}, function(index)
						{
							location.href = Helper.basePath + '/stockproduct/otherout/view/' + data.returnValue.id;
	...
	function forceCheck(id)
	{
		$.ajax({
			...
			success : function(data)
			{
				if (data.success)
				{
					location.href = Helper.basePath + '/stockproduct/otherout/view/' + id;
				} else
				{
					layer.alert("审核失败")
				}
