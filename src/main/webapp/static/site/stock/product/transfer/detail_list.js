...
{
	/* 判断当前用户是否有权限查看金额 */
	var hasPermission = Helper.basic.hasPermission('stock:productTransfer_detail:money');
	/* 选择产品 */
	...
	$("#bootTable").bootstrapTable({
		url : Helper.basePath + "/stockproduct/transfer/ajaxDetailList",
		method : "post",
		...
		cookieIdTable : "print_stockproduct_transfer_detail",// 必须制定唯一的表格cookieID
		...
		columns : [ {
			...
			formatter : function(value, row, index)
			{
				return row.master.outWarehouseName;
			}
		}, {
			...
			formatter : function(value, row, index)
			{
				return row.master.inWarehouseName;
			}
		}, {
			field : 'transferTime',
			title : '调拨日期',
			...
			formatter : function(value, row, index)
			{
				if (row.master.transferTime)
				{
					return new Date(row.master.transferTime).format("yyyy-MM-dd");
				}
			}
		}, {
			field : 'billNo',
		...
		onDblClickRow : function(row)
		{
			...
			var url = Helper.basePath + '/stockproduct/transfer/view/' + row.master.id;
			var title = "成品调拨";
