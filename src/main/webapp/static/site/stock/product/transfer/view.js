$(function()
	{
		...
		$("#detailList").bootstrapTable({
			...
			cookieIdTable : "print_stockproduct_transfer_master_view",//必须制定唯一的表格cookieID
			columns : [ {
				field : 'id',
				title : '序号',
				width : 40,
		...
		{
			window.location.href = Helper.basePath + '/stockproduct/transfer/edit/' + $(this).val();
		});
		$("#btn_back").click(function()
		{
			var url = Helper.basePath + '/stockproduct/transfer/list';
			var title = "成品库存调拨列表";
		...
		$("#btn_audit").click(function()
		{
			...
			$.ajax({
				...
				type : "POST",
				dataType : "json",
				url : Helper.basePath + '/stockproduct/transfer/check/' + $id,
				async : false,
		...
		$("#btn_audit_cancel").click(function()
		{
			...
			$.ajax({
				...
				type : "POST",
				dataType : "json",
				url : Helper.basePath + '/stockproduct/transfer/checkBack/' + $id,
				async : false,
				error : function(request)
				{
					layer.alert("Connection error");
				...
				success : function(data)
				{
					if (data.success)
					{
						...
						} else
						{
							...
							for (var i = 0; i < data.obj.length; i++)
							{
								str = str + '编号:' + data.obj[i].product.code + '   目前库存数量' + data.obj[i].qty + '\n';
							}
							Helper.message.confirm(str, function(index)
							{
								Helper.message.confirm("确认操作会引起负库存，是否允许负库存?", function(index)
		...
		$("#btn_del").click(function()
		{
			...
			Helper.message.confirm('确认要删除吗？', function(index)
			{
				$.ajax({
					...
					type : "POST",
					dataType : "json",
					url : Helper.basePath + '/stockproduct/transfer/delete/' + this_.val(),
					async : false,
					error : function(request)
					{
						//console.log(request);
					...
					success : function(data)
					{
						...
						if (data.success)
						{
							closeTabAndJump("成品库存调拨列表");
							/*  window.location.href=Helper.basePath + '/stockproduct/transfer/list'; */
						} else
						{
							layer.alert("失败");
						}
		...
		//打印模板加载
		$("#btn_print").loadTemplate('STOCK_SPT', '/stockproduct/transfer/printAjax/'+$("#transferId").val());
	...
	function forceCheckBack(id)
	{
		$.ajax({
			...
			type : "POST",
			dataType : "json",
			url : Helper.basePath + '/stockproduct/transfer/forceCheckBack/' + id,
			async : false,
			error : function(request)
			{
				layer.alert("Connection error");
