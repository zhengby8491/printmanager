 ...
 */
package com.huayin.printmanager.plugin;
...
public class PageSupport<T>
{
	...
	private int pageSize = Integer.valueOf(SystemConfigUtil.getConfig(SysConstants.DEFAULT_PAGE_SIZE)); // 页面大小，设置为“-1”表示不进行分页（分页无效）
	 ...
	 */
	public PageSupport(HttpServletRequest request, int defaultPageSize)
	{
		// 设置页码参数
	 ...
	 * @param count 数据条数
	 */
	public PageSupport(int pageNo, int pageSize, long count)
	{
	 ...
	 * @param list 本页数据对象列表
	 */
	public PageSupport(int pageNo, int pageSize, long count, List<T> list)
	{
		this.setCount(count);
	...
	PageSupport<String> p = new PageSupport<String>(3, 3);
	 ...
	 * @param list
	 */
	public PageSupport<T> setList(List<T> list)
	{
		this.list = list;
		initialize();
		return this;
...
/*	public void setSpringPage(org.springframework.data.domain.Page<T> page)
	 ...
	 * 设置 Spring data JPA 分页对象，转换为本系统分页对象
	 */
	public void setSearchResult(SearchResult<T> page)
	{
