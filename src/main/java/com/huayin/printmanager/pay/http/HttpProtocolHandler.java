...
public class HttpProtocolHandler {
    ...
    public HttpResponse execute(HttpRequest request, String strParaFileName, String strFilePath) throws HttpException, IOException {
        ...
        //get模式且不带上传文件
        if (request.getXmlPost()) {
            // xml内容提交.
            // 得指明使用UTF-8编码，否则到API服务器XML的中文不能被成功识别
            method = new PostMethod(request.getUrl());
            method.setRequestHeader("Content-Type", "text/xml");
            StringRequestEntity postEntiry  = new StringRequestEntity(request.getXmlData(), "text/xml", "UTF-8");
            ((PostMethod) method).setRequestEntity(postEntiry);
        } else if (request.getMethod().equals(HttpRequest.METHOD_GET)) {
